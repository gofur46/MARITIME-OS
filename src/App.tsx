...
export const PORT_PROFILES: Record<
  string,
  {
    avgWave: number;
    waveKet: string;
...
const buildRecordForSaving = (buffer: WeatherData[], historyData: WeatherData[]): WeatherData => {
  if (buffer.length === 0) {
    // Return default record jika buffer kosong
    return {
      timestamp: Date.now(),
      temperature: 28.0,
      humidity: 80,
  ...
  // Hitung rata-rata untuk parameter utama (avg)
  let sumTemp = 0,
    sumHum = 0,
    sumSpeed = 0,
    sumPress = 0,
  ...
  buffer.forEach((item) => {
    ...
    sumWaterTemp += item.waterTemp ?? item.temperature - 1.2;
    sumBatt += item.battery ?? 12.2;
  ...
  const avgDirection = Math.round((Math.atan2(sinSum / count, cosSum / count) * 180) / Math.PI);
  const finalDirection = avgDirection < 0 ? avgDirection + 360 : avgDirection;

  // Ambil sampel terakhir untuk mendapatkan nilai min/max dari logger (non-water-level)
  const lastSample = buffer[buffer.length - 1];

  // Hitung min/max water level dari history (24 jam terakhir)
  const { min: seaLevelMin, max: seaLevelMax } = computeWaterLevelMinMax(historyData);

  // Buat record dengan nilai rata-rata dan min/max dari logger (kecuali water level)
  return {
    timestamp: Date.now(), // akan di-override di pemanggil
    ...
    windDirection: finalDirection,
    pressure: parseFloat((sumPress / count).toFixed(1)),
    solarRadiation: Math.round(sumSolar / count),
    solarRadiationMax: Math.round(Math.max(...buffer.map((i) => i.solarRadiationMax ?? i.solarRadiation))),
    ...
    seaLevelMin: parseFloat(seaLevelMin.toFixed(1)),
    seaLevelMax: parseFloat(seaLevelMax.toFixed(1)),
    waterPh: parseFloat((sumPh / count).toFixed(2)),
    waterTemp: parseFloat((sumWaterTemp / count).toFixed(1)),
    // Min/Max dari logger (lastSample) atau fallback
    tempMin: lastSample.tempMin ?? parseFloat((lastSample.temperature - 1.5).toFixed(1)),
    tempMax: lastSample.tempMax ?? parseFloat((lastSample.temperature + 1.2).toFixed(1)),
    windSpeedMin: lastSample.windSpeedMin ?? parseFloat(Math.max(0, lastSample.windSpeed - 1.8).toFixed(1)),
    windSpeedMax: lastSample.windSpeedMax ?? parseFloat((lastSample.windSpeed + 2.5).toFixed(1)),
    waterTempMin: lastSample.waterTempMin ?? parseFloat((lastSample.waterTemp ?? lastSample.temperature - 1.2 - 0.8).toFixed(1)),
    waterTempMax: lastSample.waterTempMax ?? parseFloat((lastSample.waterTemp ?? lastSample.temperature - 1.2 + 0.5).toFixed(1)),
    battery: parseFloat((sumBatt / count).toFixed(2)),
    // windGust dihitung dari buffer (bisa tetap dipertahankan)
    windGust: (() => {
      const gusts: number[] = [];
...
const resolveLocalApiUrl = (urlStr: string) => {
  ...
  const isCloudPreview = currentHost.includes("run.app") || currentHost.includes("aistudio") || currentHost.includes("google.com");
  if (isCloudPreview) {
    return urlStr;
  }
...
export default function App() {
        ...
        if (configRes.ok) {
          ...
          setConfig(prev => {
            const newConfig = {
              ...
              sensors: prev.sensors || moxaSensors
            };
            localStorage.setItem('aws_config', JSON.stringify(newConfig));
            return newConfig;
          });
  ...
  // Data untuk grafik: gunakan database jika terhubung, fallback ke history
  const chartData = useMemo(() => {
    if (isDbConnected && realDbLogs.length > 0) {
      return realDbLogs;
    }
    return history;
      ...
      try {
        ...
        socket.on("statusUpdate", (status: any) => {
          if (status) {
            ...
            const isCloudPreview = window.location.hostname.includes("run.app") || window.location.hostname.includes("google.com") || window.location.hostname.includes("aistudio");
...
<TelemetryRecord station="${config.idStation || "AWS001"}" ts="${formatSqlDateTime(record.timestamp)}" interval="${config.dbStorageInterval || 10}m" mode="${config.dbStorageMode || "AVG"}">
  ...
  const processNewSample = (record: WeatherData) => {
    ...
    if (intervalMin === 0) {
      // INSTANT LOGGING: simpan langsung dengan water level min/max dari history
      const { min: seaLevelMin, max: seaLevelMax } = computeWaterLevelMinMax(historyRef.current);
      const recordToSave = {
        ...record,
        seaLevelMin,
        seaLevelMax,
        // pastikan min/max lainnya dari record (logger)
      };
      ...
      setStreamLogs((prevLogs) => {
        const lines = prevLogs.split("\n");
        const timeStr = format(new Date(), "HH:mm:ss");
        const msg = `[${timeStr} SQL SYSTEM] 🚀 [INSTANT LOGGING] Data dikirim ke database PostgreSQL secara real-time (tanpa buffer).`;
        const output = [...lines, msg];
        if (output.length > 40) return output.slice(output.length - 30).join("\n");
        return output.join("\n");
      });
    ...
    if (currentBlock > lastSaveBlock) {
      // Simpan data untuk blok waktu ini
      ...
      if (spaceMode === "AVG") {
        ...
        msgLog = `⏱️ Memproses dan menyimpan rata-rata standar WMO ${configRef.current.dbStorageInterval} menit berdasarkan ${updated.length} sampel ke database.`;
      } else {
        ...
        msgLog = `📦 Menyimpan data instan mentah interval ${configRef.current.dbStorageInterval} menit langsung ke database.`;
      }
  ...
  const parseIncomingSentence = (line: string, source: "SERIAL" | "TCP" | "MOXA_TCP" | "MANUAL") => {
    ...
    try {
      ...
      const ph = getMappedVal("ch_18", 7.8);
      ...
      record = {
        ...
        rainfall: rain,
        waveHeight: 1.1, // constant base wave height
        seaLevel: sea,
        waterPh: ph,
  ...
  const handlePasscodeSubmit = (e?: React.FormEvent) => {
    ...
    const matchedAccount = adminAccounts.find((acc) => acc.username.trim().toLowerCase() === loginUsernameVal.trim().toLowerCase() && acc.passcode === passcodeVal);
...
*/
  ...
  const lastGustRecord = [...history].reverse().find((row) => {
    ...
    return rowDate.getFullYear() === clockDate.getFullYear() && rowDate.getMonth() === clockDate.getMonth() && rowDate.getDate() === clockDate.getDate();
  });
  ...
  const calculateRainProbability = (current: WeatherData, logs: WeatherData[]): { probability: number; trend: string; details: string } => {
    ...
    const details = detailsList.length > 0 ? detailsList.slice(0, 3).join(", ") : "Kondisi atmosfer terpantau normal.";
  ...
  }, [warningStatus.hasWarning, warningStatus.ttsText, isAudioAlarmEnabled, isAlarmSilenced, alarmSoundType, alarmVolume, lastSpokenWarning, lastSpokenTime]);
  ...
  const exportLogsToCSV = () => {
    const headers = [
      ...
      "Water pH",
      "Water Temp (deg C)",
      "Water Temp Min (deg C)",
      "Water Temp Max (deg C)",
      "Battery (V)",
      "WindDir (deg)",
      "WindSpd (m/s)",
      "WindSpd Min (m/s)",
  ...
  return (
    <div className="min-h-screen flex flex-col md:flex-row grid-pattern text-slate-100">
      ...
      <aside className="w-full md:w-28 bg-gradient-to-b from-[#11243b] via-[#081220] to-[#03060d] border-r border-[#00f0ff]/20 flex flex-col items-center py-8 gap-8 z-10 shadow-[6px_0_50px_rgba(0,240,255,0.08)] relative">
        ...
        {/* PII Instrument Logo */}
        <div
          className="w-16 h-16 bg-gradient-to-br from-[#00f0ff]/20 to-[#3b82f6]/10 rounded-2xl flex flex-col items-center justify-center shadow-[0_0_25px_rgba(0,240,255,0.15)] border border-[#00f0ff]/30 cursor-pointer p-1.5 hover:border-[#00f0ff]/60 hover:shadow-[0_0_30px_rgba(0,240,255,0.3)] transition-all duration-300"
        ...
        <nav className="flex flex-col gap-5 w-full px-3">
          <button
            onClick={() => setActiveTab("realtime")}
            className={`w-full py-3.5 px-2 rounded-xl flex flex-col items-center gap-1.5 transition-all text-xs font-bold uppercase tracking-wider font-sans border ${
              activeTab === "realtime" ? "bg-[#00f0ff]/15 text-[#00f0ff] border-[#00f0ff]/40 shadow-[0_0_15px_rgba(0,240,255,0.15)]" : "text-slate-400 border-transparent hover:text-white hover:bg-white/5"
            }`}
          >
          ...
          <button
            ...
            className={`w-full py-3.5 px-2 rounded-xl flex flex-col items-center gap-1.5 transition-all text-xs font-bold uppercase tracking-wider font-sans border ${
              activeTab === "analyst" ? "bg-[#00f0ff]/15 text-[#00f0ff] border-[#00f0ff]/40 shadow-[0_0_15px_rgba(0,240,255,0.15)]" : "text-slate-400 border-transparent hover:text-white hover:bg-white/5"
            }`}
          >
          ...
          {currentUserRole === "admin" && (
            <>
              <button
                ...
                className={`w-full py-3.5 px-2 rounded-xl flex flex-col items-center gap-1.5 transition-all text-xs font-bold uppercase tracking-wider font-sans border relative ${
                  activeTab === "database" ? "bg-[#00f0ff]/15 text-[#00f0ff] border-[#00f0ff]/40 shadow-[0_0_15px_rgba(0,240,255,0.15)]" : "text-slate-400 border-transparent hover:text-white hover:bg-white/5"
                }`}
              >
              ...
              <button
                ...
                className={`w-full py-3.5 px-2 rounded-xl flex flex-col items-center gap-1.5 transition-all text-xs font-bold uppercase tracking-wider font-sans border relative ${
                  activeTab === "settings" ? "bg-[#00f0ff]/15 text-[#00f0ff] border-[#00f0ff]/40 shadow-[0_0_15px_rgba(0,240,255,0.15)]" : "text-slate-400 border-transparent hover:text-white hover:bg-white/5"
                }`}
              >
                <div className="relative">
          ...
          <button
            ...
            className={`w-full py-3.5 px-2 rounded-xl flex flex-col items-center gap-1.5 transition-all text-xs font-bold uppercase tracking-wider font-sans border ${
              activeTab === "bmkg" ? "bg-emerald-500/15 text-emerald-400 border-emerald-500/40 shadow-[0_0_15px_rgba(16,185,129,0.15)]" : "text-slate-400 border-transparent hover:text-white hover:bg-white/5"
            }`}
          >
        ...
        <header className="flex flex-col lg:flex-row gap-6 justify-between items-start lg:items-center bg-gradient-to-r from-[#0b1424]/90 via-[#0b1424]/50 to-bg/90 backdrop-blur-xl p-5 md:p-6 rounded-[1.5rem] border border-[#00f0ff]/20 shadow-[0_20px_50px_rgba(0,0,0,0.8)] relative">
            ...
            <div className="text-xs uppercase tracking-[0.3em] font-mono text-[#00f0ff]/80 font-extrabold flex items-center gap-2 mb-1.5">
              {config.transport !== "OFF" && !isLiveActive ? (
                ...
                <>
                  ...
                  <span>
                    AWS OS CONNECTION: {config.transport}
                    {config.transport !== "OFF" && ` (${config.serialcom || "172.16.4.48"}:${config.baudrate || "5001"})`}
                  </span>
                </>
              )}
            </div>
          ...
          <div className="flex items-center gap-6 self-stretch lg:self-auto justify-between lg:justify-end border-t lg:border-t-0 border-white/5 pt-3 lg:pt-0">
            ...
            <div className="text-right flex flex-col items-end">
              <div className="text-xs font-mono opacity-50 uppercase tracking-widest text-[#00f0ff] font-semibold">{format(currentClockTime, "EEEE, dd MMM yyyy")}</div>
              ...
              <div className="absolute inset-0 bg-[#020408]/92 backdrop-blur-md z-40 flex flex-col items-center justify-center p-6 text-center rounded-3xl border border-red-500/20">
                <div className="max-w-xl space-y-6">
                  ...
                  <div className="space-y-2">
                    <h2 className="text-2xl font-black text-red-500 tracking-tight uppercase">🔴 STATUS AWS: OFFLINE (TIDAK ADA DATA MASUK)</h2>
                    <p className="text-sm text-slate-200 font-medium">Sensor data stream terputus! Tidak ada paket data baru yang diterima dari Data Logger.</p>
                  </div>
                  ...
                  <div className="bg-[#050a12]/90 border border-red-500/15 p-4 rounded-xl font-mono text-[11px] text-left text-slate-300 space-y-2 max-w-md mx-auto shadow-inner">
                    <div className="flex justify-between border-b border-white/5 pb-1">
                      <span className="text-slate-500">Logger Transport Mode:</span> <span className="font-extrabold text-red-400">{config.transport}</span>
                    </div>
                    {config.transport === "MOXA_TCP" ? (
                      <>
                        <div className="flex justify-between border-b border-white/5 pb-1">
                          <span className="text-slate-500">IP Gateway Moxa:</span> <span className="font-bold text-slate-200">{config.serialcom || "172.16.4.48"}</span>
                        </div>
                        <div className="flex justify-between border-b border-white/5 pb-1">
                          <span className="text-slate-500">Port Gateway Moxa:</span> <span className="font-bold text-slate-200">{config.baudrate || "5001"}</span>
                        </div>
                      </>
                    ) : (
                      <div className="flex justify-between border-b border-white/5 pb-1">
                        <span className="text-slate-500">Serial Port / Endpoint:</span> <span className="font-bold text-slate-200">{config.serialcom || "COM1"}</span>
                      </div>
                    )}
                    <div className="flex justify-between border-b border-white/5 pb-1">
                      <span className="text-slate-500">Waktu Timeout Sistem:</span> <span className="font-bold text-amber-500">25 Detik Tanpa Data</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-slate-500">Status Jalur Data:</span> <span className="font-extrabold text-red-500 animate-pulse">🔴 TERMINATED / NO CONNECTION</span>
                    </div>
                  </div>
            ...
            {warningStatus.hasWarning && (
              <div
                className={`mb-6 bg-gradient-to-r ${
                  warningStatus.isStormHazard
                    ? "from-red-950/80 via-red-900/60 to-red-950/80 border-red-500/50 shadow-[0_0_20px_rgba(239,68,68,0.25)]"
                    : "from-amber-950/80 via-amber-900/60 to-amber-950/80 border-amber-500/50 shadow-[0_0_20px_rgba(245,158,11,0.2)]"
                } border-2 rounded-2xl p-4.5 flex flex-col lg:flex-row items-center justify-between gap-4 select-none`}
              >
                <div className="flex items-center gap-3">
                  ...
                  <div className="text-left space-y-1">
                    <span className="text-xs font-black uppercase tracking-widest text-[#00f0ff] font-mono flex items-center gap-1.5">🚨 SISTEM SIRENE & AUDIO ALARM AKTIF (EWS)</span>
                    <div className="flex flex-wrap gap-1.5 items-center mt-1">
                      {warningStatus.warnings.map((warn, i) => (
                        <span
                          key={i}
                          className={`text-[10px] font-extrabold px-2.5 py-1 rounded uppercase font-mono tracking-wider ${
                            warningStatus.isStormHazard ? "bg-red-500/30 text-red-200 border border-red-500/50" : "bg-amber-500/30 text-amber-200 border border-amber-500/50"
                          } animate-pulse`}
                        >
                          ⚠️ {warn}
                        </span>
                ...
                <div className="flex flex-wrap sm:flex-nowrap gap-2.5 w-full lg:w-auto items-center">
                  ...
                  {isAudioAlarmEnabled && (
                    <button
                      ...
                      className={`w-full sm:w-auto font-mono text-[10px] font-black tracking-widest px-4 py-2.5 rounded-xl uppercase transition-all whitespace-nowrap cursor-pointer flex items-center justify-center gap-1.5 border ${
                        isAlarmSilenced ? "bg-emerald-500/20 hover:bg-emerald-500/30 text-emerald-400 border-emerald-500/30" : "bg-rose-500 hover:bg-rose-600 text-white border-transparent shadow-[0_0_15px_rgba(239,68,68,0.35)]"
                      }`}
                    >
                  ...
                  <button
                    ...
                    className={`w-full sm:w-auto font-mono text-[10px] font-black tracking-widest px-4 py-2.5 rounded-xl uppercase transition-all whitespace-nowrap cursor-pointer border ${
                      isAudioAlarmEnabled
                        ? "bg-slate-800 hover:bg-slate-700 text-slate-300 border-white/10"
                        : "bg-gradient-to-r from-teal-500 to-emerald-500 hover:from-teal-400 hover:to-emerald-400 text-black border-transparent shadow-[0_0_15px_rgba(20,184,166,0.3)]"
                    }`}
                  >
            ...
            <div className={`grid grid-cols-1 lg:grid-cols-12 gap-6 items-stretch ${config.transport !== "OFF" && !isLiveActive && config.lockOfflineDashboard !== "OFF" ? "pointer-events-none opacity-30 filter blur-[2px]" : ""}`}>
              ...
              <div className="lg:col-span-3 flex flex-col space-y-4 h-full">
                ...
                <div className="bg-gradient-to-b from-[#0b1424]/40 to-bg p-4.5 rounded-2xl border border-sky-400/20 flex-1 flex flex-col justify-between space-y-3 font-sans shadow-[0_0_12px_rgba(56,189,248,0.05)]">
                  ...
                  <div className="flex-1 flex flex-col justify-between gap-2">
                    ...
                    <div className="bg-[#0b1424] border border-white/5 p-2.5 flex justify-between items-center transition-colors hover:border-white/10 rounded-xl">
                      ...
                      <div className="text-right">
                        <span className="text-xl font-bold font-mono text-white">{computeDewPoint(currentData.temperature, currentData.humidity)}</span>
                    ...
                    <div className="bg-[#0b1424] border border-white/5 p-2.5 flex justify-between items-center transition-colors hover:border-white/10 rounded-xl">
                      ...
                      <div className="text-right flex flex-col items-end">
                        ...
                        <span className="text-[10px] text-sky-500/80 font-mono">Max: {currentData.solarRadiationMax ?? Math.round(currentData.solarRadiation * 1.15)} W/m²</span>
                      </div>
                    </div>
                    ...
                    {(() => {
                      ...
                      return (
                        <div
                          className={`bg-[#0b1424] border rounded-xl p-2.5 flex justify-between items-center transition-colors ${
                            isHeavyRain ? "border-red-500/50 bg-red-950/20 shadow-[0_0_10px_rgba(239,68,68,0.15)] animate-pulse" : "border-white/5 hover:border-white/10"
                          }`}
                        >
                      ...
                      return (
                        <div className="bg-[#050a12]/80 border border-white/5 p-2.5 rounded-xl flex flex-col space-y-1.5 mt-0.5">
                          <div className="flex justify-between items-center">
                            <span className="text-[10px] uppercase font-bold text-sky-400/80 tracking-wider">Peluang Hujan (1 Jam Terakhir Trend)</span>
                            <span className={`text-[10px] px-1.5 py-0.2 rounded border font-mono font-bold ${badgeColor}`}>{rainPred.probability}%</span>
                          </div>
                          {/* Progress Bar */}
                          <div className="w-full bg-slate-800/60 h-1.5 rounded-full overflow-hidden">
                            <div className={`h-full rounded-full transition-all duration-500 ${progressColor}`} style={{ width: `${rainPred.probability}%` }} />
                          </div>
                          {/* Details */}
                          <div className="text-[10px] leading-relaxed text-slate-400 flex flex-col space-y-0.5">
                            <span className="font-bold text-white text-[10px] block truncate">Status: {rainPred.trend}</span>
                            <span className="text-slate-400 block text-[9.5px] leading-tight">Analisis: {rainPred.details}</span>
                          </div>
                        </div>
                      );
                    })()}
                ...
                {(() => {
                  ...
                  return (
                    <div
                      className={`bg-gradient-to-b from-[#0b1424]/40 to-bg p-4.5 rounded-2xl border transition-all ${
                        isPhUnsafe ? "border-red-500/50 shadow-[0_0_15px_rgba(239,68,68,0.15)] animate-pulse" : "border-teal-400/20 shadow-[0_0_12px_rgba(20,184,166,0.05)]"
                      } flex-1 flex flex-col justify-between space-y-3`}
                    >
                      <div>
                        <div
                          className={`text-xs font-bold ${isPhUnsafe ? "text-red-500" : "text-teal-400"} uppercase tracking-[0.2em] flex items-center justify-between border-b border-white/5 pb-2 ${
                            !isPhUnsafe && "drop-shadow-[0_0_6px_rgba(20,184,166,0.3)]"
                          }`}
                        >
                      ...
                      <div className="flex-1 flex flex-col justify-center">
                        <div className="bg-[#050a12]/80 border border-white/5 p-3 rounded-xl space-y-2">
                          ...
                          <div className="grid grid-cols-2 gap-2 text-center">
                            ...
                            <div
                              className={`bg-[#0b1424] border py-1.5 px-2 rounded-lg flex flex-col justify-center items-center transition-colors ${
                                isPhUnsafe ? "border-red-500/20 hover:border-red-500/30" : "border-white/5 hover:border-white/10"
                              }`}
                            >
              ...
              <div className="lg:col-span-5 bg-gradient-to-br from-[#0d1726]/80 to-bg border border-[#00f0ff]/20 p-6 rounded-3xl relative min-h-[500px] flex flex-col justify-between shadow-[0_30px_70px_rgba(0,0,0,0.9)] h-full">
                ...
                <div className="text-center font-bold">
                  ...
                  <div className="flex flex-wrap items-center justify-center gap-3 mt-1.5">
                    <span className="text-xs font-mono text-[#00f0ff]/70 uppercase tracking-widest bg-[#00f0ff]/5 py-0.5 px-3 rounded border border-[#00f0ff]/10">CONSOLE_INTEGRATION_ONLINE</span>
                    <div className="flex items-center bg-[#050a12]/80 border border-[#00f0ff]/30 rounded-lg p-0.5">
                      <button
                        ...
                        className={`px-2 py-0.5 rounded text-[10px] font-mono transition-all duration-200 ${
                          windUnit === "ms" ? "bg-[#00f0ff] text-slate-900 font-extrabold shadow-[0_0_8px_rgba(0,240,255,0.5)]" : "text-[#00f0ff]/60 hover:text-[#00f0ff]"
                        }`}
                      >
                        m/s
                      </button>
                      <button
                        ...
                        className={`px-2 py-0.5 rounded text-[10px] font-mono transition-all duration-200 ${
                          windUnit === "kt" ? "bg-[#00f0ff] text-slate-900 font-extrabold shadow-[0_0_8px_rgba(0,240,255,0.5)]" : "text-[#00f0ff]/60 hover:text-[#00f0ff]"
                        }`}
                      >
                        KNOT
                      </button>
                  ...
                  if (isStormHazard) {
                    ...
                    return (
                      <div className="mx-auto mt-2.5 px-4 py-1.5 border border-white/10 bg-white/5 rounded-xl flex items-center gap-2 justify-center max-w-xs select-none">
                        ...
                        <span className="text-xs font-bold text-white uppercase tracking-wider font-mono text-center">🟢 STATUS OPERASI: AMAN & NORMAL</span>
                      </div>
                    );
                  }
                })()}
                  ...
                  {(() => {
                    ...
                    return (
                      <div className={`relative w-80 h-80 rounded-full border-8 transition-all duration-700 flex items-center justify-center bg-transparent shadow-[inset_0_0_40px_rgba(0,0,0,0.85)] ${ringBorderColor}`}>
                        ...
                        <div
                          className={`absolute w-20 h-20 rounded-full bg-[#030712]/95 border-2 flex flex-col items-center justify-center shadow-[0_0_18px_rgba(255,255,255,0.05)] z-30 font-mono pointer-events-none transition-all duration-300 ${
                            hasWarning ? "border-red-500/50 shadow-[0_0_18px_rgba(239,68,68,0.25)]" : "border-white/20"
                          }`}
                        >
                          ...
                          <span className={`text-xl font-black leading-none mb-0.5 ${hasWarning ? "text-red-500" : "text-white"}`}>{windUnit === "ms" ? currentData.windSpeed.toFixed(1) : (currentData.windSpeed * 1.94384).toFixed(1)}</span>
                          <span className="text-[8px] text-slate-400 font-sans leading-none font-bold text-center">{windUnit === "ms" ? "m/s" : "knot"}</span>
                        </div>

                        {/* Port/Darat vs Sea/Open Water Boundary Divider Line rotated with visual pierAngle state */}
                        <div className="absolute w-full h-full flex items-center justify-center transition-all duration-1000 ease-out z-10 pointer-events-none" style={{ transform: `rotate(${config.pierAngle}deg)` }}>
                          <div className="relative w-full h-full flex items-center justify-center">
                            ...
                            <div className="absolute left-[40px] top-[110px] text-[7.5px] uppercase font-black text-white/40 font-sans tracking-[0.25em] -rotate-90">LAUT LEPAS / OPEN SEA</div>

                            {/* Right side region: DERMAGA / AREA DARAT */}
                            <div className="absolute right-[40px] top-[110px] text-[7.5px] uppercase font-black text-white/40 font-sans tracking-[0.25em] rotate-90">DERMAGA / AREA DARAT</div>
                          </div>
                        </div>

                        {/* Pointer rotating Wind Arrow strictly matching live wind arah direction */}
                        <div className="absolute w-full h-full flex items-center justify-center transition-all duration-1000 ease-out pointer-events-none z-20" style={{ transform: `rotate(${currentData.windDirection}deg)` }}>
                          ...
                          <div className="absolute top-[4px] bottom-[4px] w-[2px] bg-gradient-to-b from-[#00ff66]/70 via-[#00ff66]/10 to-transparent flex flex-col items-center">
                            ...
                            <svg className="w-12 h-12 text-[#00ff66] fill-[#00ff66]/25 drop-shadow-[0_0_15px_#00ff66] -mt-3" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round">
                ...
                <div className="grid grid-cols-2 gap-4 items-center mb-6 max-w-md mx-auto bg-[#050a12]/70 p-3.5 rounded-2xl border border-[#00f0ff]/20 text-center font-mono text-xs shadow-[0_0_12px_rgba(0,240,255,0.05)]">
                  ...
                  <div className="pl-2">
                    ...
                    <span className="text-sm font-extrabold text-white">
                      {currentData.windDirection}° ({getWindRoseString(currentData.windDirection)})
                    </span>
                  </div>
                </div>
                  ...
                  <div className="grid grid-cols-4 gap-2.5">
                    ...
                    <div className="bg-[#050a12] border border-white/5 p-3 rounded-xl text-center flex flex-col justify-center items-center">
                      ...
                      <span className="text-[10px] font-bold text-[#00f0ff]/80 font-sans mt-0.5">
                        Pasut: {currentData.seaLevel / 100 - calcMslHeight >= 0 ? "+" : ""}
                        {(currentData.seaLevel / 100 - calcMslHeight).toFixed(3)}m
                      </span>
                    </div>
                  </div>
                </div>
              ...
              <div className="lg:col-span-4 flex flex-col space-y-4 h-full">
                ...
                <div className="bg-gradient-to-b from-[#0b1424]/40 to-bg p-3.5 rounded-2xl border border-sky-500/20 flex-1 flex flex-col justify-between space-y-2 shadow-[0_0_12px_rgba(56,189,248,0.05)] font-sans">
                  ...
                  <div className="grid grid-cols-3 gap-2 font-sans flex-1 flex items-center">
                    <div className="bg-[#0b1424] border border-sky-500/10 p-2 rounded-xl text-center w-full">
                      <span className="text-[9px] uppercase tracking-wider text-sky-400 font-extrabold block mb-0.5 truncate" title="Barometer">
                        Barometer
                      </span>
                      <span className="text-xs font-black font-mono text-white block">
                        {currentData.pressure.toFixed(1)} <span className="text-[8px] font-sans text-sky-400/80 font-normal w-full">hPa</span>
                      </span>
                    </div>
                    <div className="bg-[#0b1424] border border-sky-500/10 p-2 rounded-xl text-center w-full">
                      <span className="text-[9px] uppercase tracking-wider text-sky-400 font-extrabold block mb-0.5 truncate" title="QFF">
                        QFF
                      </span>
                      <span className="text-xs font-black font-mono text-white block">
                        {(currentData.pressure + 2.1).toFixed(1)} <span className="text-[8px] font-sans text-sky-400/80 font-normal w-full">hPa</span>
                      </span>
                    </div>
                    <div className="bg-[#0b1424] border border-sky-500/10 p-2 rounded-xl text-center w-full">
                      <span className="text-[9px] uppercase tracking-wider text-sky-400 font-extrabold block mb-0.5 truncate" title="QFE">
                        QFE
                      </span>
                      <span className="text-xs font-black font-mono text-white block">
                        {currentData.pressure.toFixed(1)} <span className="text-[8px] font-sans text-sky-400/80 font-normal w-full">hPa</span>
                ...
                <div className="bg-gradient-to-b from-[#0b1424]/40 to-bg p-3.5 rounded-2xl border border-sky-500/20 flex-1 flex flex-col justify-between space-y-2 shadow-[0_0_12px_rgba(56,189,248,0.05)]">
                  <div className="text-xs font-bold text-sky-400 uppercase tracking-[0.2em] flex items-center justify-between border-b border-white/5 pb-2 drop-shadow-[0_0_6px_rgba(56,189,248,0.3)]">
                    <div className="flex items-center gap-2">
                      {currentData.solarRadiation > 50 ? <BatteryCharging className="w-3.5 h-3.5 text-sky-400 animate-pulse" /> : <Battery className="w-3.5 h-3.5 text-sky-400" />}
                    ...
                    return (
                      <div className="grid grid-cols-12 gap-3 items-center font-sans">
                        ...
                        <div className="col-span-4 text-center">
                          <div className={`border p-1.5 rounded-lg font-mono text-[10px] font-bold uppercase ${statusColor} bg-[#0b1424]`}>{statusText}</div>
                  ...
                  <div className="flex flex-row items-center justify-between gap-3.5 py-1.5">
                      ...
                      {(() => {
                        ...
                        return (
                          <svg viewBox="0 0 200 200" className="w-[220px] h-[220px] sm:w-[250px] sm:h-[250px] select-none font-sans">
                            ...
                            {/* Ring labels */}
                            <text x={cx + 2} y={cy - maxR + 8} fill="rgba(255,255,255,0.4)" fontSize={5.5} className="font-mono font-bold">
                              {maxPctScope.toFixed(0)}%
                            </text>
                            <text x={cx + 2} y={cy - maxR * 0.5 + 6} fill="rgba(255,255,255,0.25)" fontSize={5.5} className="font-mono">
                              {(maxPctScope / 2).toFixed(0)}%
                            </text>

                            {/* Cardinal Labels */}
                            <text x={cx} y={cy - maxR - 4} fill="#f8fafc" fontSize={7} fontWeight="bold" textAnchor="middle" alignmentBaseline="middle">
                              N
                            </text>
                            <text x={cx + maxR + 5} y={cy} fill="#94a3b8" fontSize={7} fontWeight="bold" textAnchor="start" alignmentBaseline="middle">
                              E
                            </text>
                            <text x={cx} y={cy + maxR + 5} fill="#94a3b8" fontSize={7} fontWeight="bold" textAnchor="middle" alignmentBaseline="middle">
                              S
                            </text>
                            <text x={cx - maxR - 5} y={cy} fill="#94a3b8" fontSize={7} fontWeight="bold" textAnchor="end" alignmentBaseline="middle">
                              W
                            </text>
                            ...
                            {(() => {
                              ...
                              return (
                                <>
                                  <text x={ne.x} y={ne.y} fill="rgba(255,255,255,0.15)" fontSize={5} textAnchor="middle" alignmentBaseline="middle">
                                    NE
                                  </text>
                                  <text x={se.x} y={se.y} fill="rgba(255,255,255,0.15)" fontSize={5} textAnchor="middle" alignmentBaseline="middle">
                                    SE
                                  </text>
                                  <text x={sw.x} y={sw.y} fill="rgba(255,255,255,0.15)" fontSize={5} textAnchor="middle" alignmentBaseline="middle">
                                    SW
                                  </text>
                                  <text x={nw.x} y={nw.y} fill="rgba(255,255,255,0.15)" fontSize={5} textAnchor="middle" alignmentBaseline="middle">
                                    NW
                                  </text>
                                </>
                              );
                            ...
                            {matrix.map((binsInSector, dIdx) => {
                              ...
                              return binsInSector.map((countInBin, bIdx) => {
                                ...
                                return (
                                  <path key={`${dIdx}-${bIdx}`} d={path} fill={colors[bIdx]} opacity={0.88} className="transition-all duration-300 hover:opacity-100 hover:stroke-white/30 hover:stroke-[0.5px]">
                                    <title>{`Arah: ${["N", "NNE", "NE", "ENE", "E", "ESE", "SE", "SSE", "S", "SSW", "SW", "WSW", "W", "WNW", "NW", "NNW"][dIdx]} | Kecepatan: ${
                                      ["0-4 m/s", "4-6 m/s", "6-10 m/s", "10-15 m/s", "15-20 m/s", "20-25 m/s", ">25 m/s"][bIdx]
                                    } | Proporsi: ${((countInBin / totalLogs) * 100).toFixed(1)}%`}</title>
                                  </path>
                                );
                              });
                            })}
                    ...
                    <div className="flex-1 max-w-[140px] bg-[#050a12]/50 border border-white/5 rounded-xl p-2.5 flex flex-col justify-center space-y-2 font-mono text-[10px] md:text-sm text-slate-300 font-extrabold shadow-sm select-none">
                      ...
                      <div className="flex items-center gap-2 hover:text-white transition-colors" title="Speed 0 to 4 m/s">
                        ...
                        <span>
                          0 – 4 <span className="text-[9px] font-sans text-slate-500 font-bold">m/s</span>
                        </span>
                      </div>
                      <div className="flex items-center gap-2 hover:text-white transition-colors" title="Speed 4 to 6 m/s">
                        ...
                        <span>
                          4 – 6 <span className="text-[9px] font-sans text-slate-500 font-bold">m/s</span>
                        </span>
                      </div>
                      <div className="flex items-center gap-2 hover:text-white transition-colors" title="Speed 6 to 10 m/s">
                        ...
                        <span>
                          6 – 10 <span className="text-[9px] font-sans text-slate-500 font-bold">m/s</span>
                        </span>
                      </div>
                      <div className="flex items-center gap-2 hover:text-white transition-colors" title="Speed 10 to 15 m/s">
                        ...
                        <span>
                          10 – 15 <span className="text-[9px] font-sans text-slate-500 font-bold">m/s</span>
                        </span>
                      </div>
                      <div className="flex items-center gap-2 hover:text-white transition-colors" title="Speed 15 to 20 m/s">
                        ...
                        <span>
                          15 – 20 <span className="text-[9px] font-sans text-slate-500 font-bold">m/s</span>
                        </span>
                      </div>
                      <div className="flex items-center gap-2 hover:text-white transition-colors" title="Speed 20 to 25 m/s">
                        ...
                        <span>
                          20 – 25 <span className="text-[9px] font-sans text-slate-500 font-bold">m/s</span>
                        </span>
                      </div>
                      <div className="flex items-center gap-2 hover:text-white transition-colors" title="Speed above 25 m/s">
                        ...
                        <span>
                          &gt; 25 <span className="text-[9px] font-sans text-slate-500 font-bold">m/s</span>
                        </span>
                      </div>
                    </div>
                  </div>
                      ...
                      <div className="bg-[#050a12]/80 border border-white/5 p-3 rounded-xl space-y-2">
                        ...
                        <div className="grid grid-cols-2 gap-2 text-center font-sans">
                          ...
                          <div className="bg-[#0b1424] border border-[#00f0ff]/10 py-1.5 px-2 rounded-lg flex flex-col justify-center items-center transition-colors hover:border-[#00f0ff]/20">
                            ...
                            <span className="text-xs md:text-sm font-bold font-mono text-[#00f0ff] truncate">{lastWindGustTime !== null ? lastWindGustTime : "—"}</span>
                          </div>
                        </div>
                      </div>
                    </div>
                  ...
                  {history.slice(-5).map((row, idx) => {
                    ...
                    return (
                      <div key={idx} className="bg-[#040911] border border-white/5 p-4 rounded-2xl flex flex-col items-center justify-between space-y-3 text-center transition-all hover:border-[#00f0ff]/15">
                        {/* Timestamp */}
                        <span className="text-xs text-slate-500 font-mono font-bold">{format(row.timestamp, "HH:mm")}</span>

                        {/* Rotating wind arrow icon */}
                        <div className="p-2 bg-[#02060c] rounded-xl border border-white/5 flex items-center justify-center">
                          <Navigation className="w-4 h-4 text-[#00f0ff]" style={{ transform: `rotate(${row.windDirection}deg)` }} />
                        </div>

                        {/* Cardinal Abbreviation */}
                        <span className="text-base font-black text-white font-mono tracking-wide">{directionName}</span>
          ...
          <div className="space-y-6">
            ...
            <div className="bg-gradient-to-b from-[#0b1424] to-bg p-5 rounded-2xl border border-white/10 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-5">
              <div className="flex flex-wrap gap-4 items-end">
                ...
                <button onClick={filterLogsData} className="bg-white hover:bg-slate-200 transition text-black text-xs font-bold font-mono py-2 px-5 rounded-lg uppercase flex items-center justify-center gap-2 cursor-pointer h-9">
              ...
              <div className="flex flex-wrap items-center gap-3 self-stretch sm:self-auto justify-end">
                ...
                <div className={`flex items-center gap-2.5 border rounded-xl py-1.5 px-3 h-10 ${aiForecastResult.stormBg} select-none shadow-[0_0_15px_rgba(168,85,247,0.08)]`}>
                  <div className={`text-xl ${aiForecastResult.stormPulse}`}>{aiForecastResult.stormIcon}</div>
                  <div className="text-left font-sans">
                    ...
                    <div className="text-[11px] font-black font-mono text-white flex items-center gap-1.5">
                      ...
                      <span className="text-[10px] uppercase font-black tracking-tight" style={{ color: aiForecastResult.stormColor }}>
            ...
            <div className="bg-gradient-to-b from-[#0b1424] to-bg border border-white/15 p-6 rounded-2xl shadow-none space-y-4">
              ...
              <div className="bg-[#040914] border border-[#00f0ff]/10 rounded-xl p-4 flex justify-center items-center w-full">
                <svg viewBox="0 0 900 200" className="w-full h-auto text-[8px] font-mono select-none">
                  ...
                  <text x="450" y="189" fill="#cbd5e1" textAnchor="middle" className="font-bold text-[7px] tracking-wide">
                    DASAR LAUT / SEABED GEOMETRIC REFERENCE
                  </text>
                  ...
                  <text x="65" y="37" fill="#e2e8f0" textAnchor="middle" className="font-extrabold text-[8.5px]">
                    LANTAI DERMAGA
                  </text>
                  <text x="65" y="47" fill="#94a3b8" textAnchor="middle" className="text-[6.5px]">
                    REF: 0 cm (Top)
                  </text>
                  ...
                  <text x="132" y="52" fill="#10b981" className="text-[7px] font-black font-mono">
                    SENSOR LIVE
                  </text>
                  ...
                  {(() => {
                    ...
                    return (
                      <>
                        ...
                        <text x="135" y={mslY - 4} fill="#10b981" className="text-[7px] font-bold font-mono">
                          MSL LINE ({calcMslHeight.toFixed(1)}m REF)
                        </text>

                        {/* Water Surface label */}
                        <text x="790" y={waterY - 5} fill="#38bdf8" textAnchor="middle" className="font-bold text-[8px] font-mono">
                          WATER SURFACE ({calcLoggerHeight.toFixed(2)}m)
                        </text>
                        ...
                        <text x="6" y="102" fill="#94a3b8" transform="rotate(-90 6 102)" textAnchor="middle" className="text-[7.5px] font-bold">
                          H = {calcTotalDepth.toFixed(1)}m
                        </text>
                        ...
                        <text x="878" y={(waterY + 180) / 2 + 3} fill="#f59e0b" textAnchor="end" className="font-bold text-[7.5px]">
                        ...
                        <text x="151" y={(25 + waterY) / 2 + 3} fill="#38bdf8" textAnchor="start" className="font-bold text-[7.5px]">
                          d = {(calcTotalDepth - calcLoggerHeight).toFixed(2)}m
                        </text>
                      </>
                    );
                  })()}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mt-4 border-t border-white/5 pt-4">
                ...
                <div className="bg-black/40 border border-white/5 rounded-xl p-3.5 space-y-2.5 flex flex-col justify-between">
                  <div>
                    <div className="flex items-center justify-between border-b border-white/5 pb-1 mb-2">
                      <span className="text-[9px] uppercase font-bold text-[#00f0ff] tracking-wider font-mono">🛠️ 1. Parameter H &amp; MSL</span>
                      {calcTotalDepth === parseFloat(localStorage.getItem("tide_total_depth") || "4.0") && calcMslHeight === parseFloat(localStorage.getItem("tide_msl_height") || "2.0") ? (
                ...
                <div className="bg-black/40 border border-white/5 rounded-xl p-3.5 space-y-2 flex flex-col justify-between">
                  <div>
                    <div className="flex justify-between items-center border-b border-white/5 pb-1 mb-1.5">
                      <span className="text-[9px] uppercase font-bold text-[#f59e0b] tracking-wider font-mono">🔄 2. Simulasi Pasut</span>
                      <span className="text-[10px] text-[#f59e0b] font-bold font-mono bg-[#f59e0b]/5 px-2 py-0.5 rounded border border-[#f59e0b]/10">h = {calcLoggerHeight.toFixed(2)}m</span>
                    </div>
                ...
                {/* Col 3: Hasil & Metrik */}
                <div className="bg-black/40 border border-white/5 rounded-xl p-3.5 space-y-2 flex flex-col justify-between">
                  <div className="text-[9px] uppercase font-bold text-[#00f0ff]/80 tracking-wider font-mono border-b border-white/5 pb-1">📊 3. Hasil &amp; Metrik</div>
                  ...
                  <div className="grid grid-cols-3 gap-1 font-mono text-[9px]">
                    ...
                    <div className="bg-slate-900/50 p-1 rounded border border-white/5 flex flex-col justify-center items-center text-center">
                      ...
                      <span className={`font-black text-[10px] leading-none ${calcLoggerHeight - calcMslHeight > 0.5 ? "text-pink-400" : calcLoggerHeight - calcMslHeight < -0.5 ? "text-cyan-400" : "text-emerald-400"}`}>
                        {calcLoggerHeight - calcMslHeight >= 0 ? "+" : ""}
                        {(calcLoggerHeight - calcMslHeight).toFixed(2)}m
                      </span>
                    </div>
                    <div className="bg-slate-900/50 p-1 rounded border border-white/5 flex flex-col justify-center items-center text-center">
                  ...
                  <div className="text-[8px] leading-tight text-slate-400 flex items-center gap-1">
                    <span>Status:</span>
                    <strong className={`font-bold ${calcLoggerHeight - calcMslHeight > 0.5 ? "text-pink-400" : calcLoggerHeight - calcMslHeight < -0.5 ? "text-cyan-400" : "text-emerald-400"}`}>
                ...
                <div className="bg-black/40 border border-[#00f0ff]/10 rounded-xl p-3 flex flex-col justify-center text-[9px] leading-relaxed text-slate-300 font-sans">
                  ...
                  <p className="text-slate-400 text-[8.5px] leading-snug">
                    Tinggi dermaga ke dasar laut <strong>H = {calcTotalDepth.toFixed(1)}m</strong>. Mean Sea Level (MSL) dihitung rata-rata pasut (<strong>{calcMslHeight.toFixed(1)}m</strong>). Deviasi dari MSL menunjukkan elevasi pasut
                    aktif.
                  </p>
                </div>
              </div>
            ...
            <div className="bg-gradient-to-b from-[#0b1424] to-bg border border-white/10 rounded-2xl p-6 space-y-4 shadow-xl">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between pb-3 border-b border-white/5 gap-2">
                <div className="space-y-1">
                  <h4 className="text-xs uppercase font-extrabold text-[#00f0ff] tracking-[0.2em] font-mono flex items-center gap-2">📈 HISTORI ELEVASI PASUT &amp; WATER LEVEL 24 JAM TERAKHIR (FULL WIDTH)</h4>
                </div>
                <div className="flex flex-wrap items-center gap-3 font-mono text-[9px] bg-black/30 px-3 py-1.5 rounded-lg border border-white/5">
                  ...
                  {(() => {
                    ...
                    if (diff > 0.05) {
                      return <span className="bg-pink-500/20 text-pink-400 border border-pink-500/30 px-2 py-0.5 rounded font-black tracking-wider text-[9px] animate-pulse">📈 PASANG (+{diff.toFixed(2)}m)</span>;
                    } else if (diff < -0.05) {
                      return <span className="bg-cyan-500/20 text-cyan-400 border border-cyan-500/30 px-2 py-0.5 rounded font-black tracking-wider text-[9px] animate-pulse">📉 SURUT ({diff.toFixed(2)}m)</span>;
                    } else {
                      return <span className="bg-emerald-500/20 text-emerald-400 border border-emerald-500/30 px-2 py-0.5 rounded font-black tracking-wider text-[9px]">🟢 BALANCE</span>;
                    }
                  })()}
                </div>
              </div>
              ...
              <div className="lg:col-span-4 bg-gradient-to-b from-[#0b1424] to-bg border border-white/10 p-5 rounded-2xl flex flex-col justify-between">
                  ...
                  {(() => {
                    ...
                    return (
                      <svg viewBox="0 0 200 200" className="w-[170px] h-[170px] select-none font-sans">
                        ...
                        <text x={cx + 2} y={cy - maxR + 8} fill="rgba(0,240,255,0.4)" fontSize={5.5} className="font-mono font-bold">
                          {maxPctScope.toFixed(0)}%
                        </text>
                        <text x={cx + 2} y={cy - maxR * 0.5 + 6} fill="rgba(255,255,255,0.25)" fontSize={5.5} className="font-mono">
                          {(maxPctScope / 2).toFixed(0)}%
                        ...
                        {(() => {
                          ...
                          return (
                            <>
                              <text x={ne.x} y={ne.y} fill="rgba(255,255,255,0.12)" fontSize={5} textAnchor="middle" alignmentBaseline="middle">
                                NE
                              </text>
                              <text x={se.x} y={se.y} fill="rgba(255,255,255,0.12)" fontSize={5} textAnchor="middle" alignmentBaseline="middle">
                                SE
                              </text>
                              <text x={sw.x} y={sw.y} fill="rgba(255,255,255,0.12)" fontSize={5} textAnchor="middle" alignmentBaseline="middle">
                                SW
                              </text>
                              <text x={nw.x} y={nw.y} fill="rgba(255,255,255,0.12)" fontSize={5} textAnchor="middle" alignmentBaseline="middle">
                                NW
                              </text>
                            </>
                          );
                        ...
                        {matrix.map((binsInSector, dIdx) => {
                          ...
                          return binsInSector.map((countInBin, bIdx) => {
                            ...
                            const p4 = getXY(rStart, angleStart);
                            const path = `M ${p1.x} ${p1.y} A ${rEnd} ${rEnd} 0 0 1 ${p2.x} ${p2.y} L ${p3.x} ${p3.y} A ${rStart} ${rStart} 0 0 0 ${p4.x} ${p4.y} Z`;
                            return <path key={`${dIdx}-${bIdx}`} d={path} fill={colors[bIdx]} opacity={0.88} className="transition-all duration-300 hover:opacity-100" />;
                          });
                        })}
                      </svg>
                    );
                ...
                <div className="grid grid-cols-4 gap-y-1 text-[8px] text-slate-400 border-t border-white/5 pt-2 font-mono">
                  <div className="flex items-center gap-1">
                    <span className="w-1.5 h-1.5 rounded-full bg-[#4a628a]" />
                    <span className="truncate">0-4 kt</span>
                  </div>
                  <div className="flex items-center gap-1">
                    <span className="w-1.5 h-1.5 rounded-full bg-[#22c55e]" />
                    <span className="truncate">4-6 kt</span>
                  </div>
                  <div className="flex items-center gap-1">
                    <span className="w-1.5 h-1.5 rounded-full bg-[#eab308]" />
                    <span className="truncate">6-10 kt</span>
                  </div>
                  <div className="flex items-center gap-1">
                    <span className="w-1.5 h-1.5 rounded-full bg-[#f97316]" />
                    <span className="truncate">10-15</span>
                  </div>
                  <div className="flex items-center gap-1">
                    <span className="w-1.5 h-1.5 rounded-full bg-[#db2777]" />
                    <span className="truncate">15-20</span>
                  </div>
                  <div className="flex items-center gap-1">
                    <span className="w-1.5 h-1.5 rounded-full bg-[#7c3aed]" />
                    <span className="truncate">20-25</span>
                  </div>
                  <div className="flex items-center gap-1 col-span-2">
                    <span className="w-1.5 h-1.5 rounded-full bg-[#2563eb]" />
                    <span className="truncate">&gt;25 kt</span>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="bg-gradient-to-r from-[#0a1b3a] to-[#041026] p-5 rounded-2xl border border-teal-500/30 shadow-[0_0_20px_rgba(20,184,166,0.1)] space-y-4">
              <div className="flex flex-wrap items-center justify-between gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-400 mt-1 max-w-[650px]">
                      {config.dbStorageMode === "AVG" ? (
                        <span>
                          The system is configured in accordance with WMO meteorology standards utilizing a compressed **{config.dbStorageInterval}-Minute Average** interval, minimizing query overhead and ensuring high database efficiency.
                        </span>
                      ) : (
                        <span>
                          The system is configured in **Instantaneous/Raw Storage Mode**, recording asynchronous sensor samples directly to the SQL server without averaging statistics at **{config.dbStorageInterval}-Minute** intervals.
                        </span>
                      )}
                    </p>
                  </div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="px-3 py-1 rounded-full text-xs font-black uppercase font-mono tracking-widest bg-indigo-500/15 text-indigo-300 border border-indigo-500/25 animate-pulse">🐘 POSTGRESQL DRIVER ACTIVE</span>
                </div>
              </div>
              ...
              <div className="p-4 bg-teal-500/5 rounded-xl border border-teal-500/15 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
                <div className="space-y-1">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs uppercase tracking-wider font-black text-teal-400 font-mono">CYCLE ACCUMULATOR BUFFER ({sampleBuffer.length} / 5)</span>
                  </div>
                  <p className="text-xs text-slate-300">
                    {config.dbStorageMode === "AVG" ? (
                      <span>
                        The WMO compliant average is mathematically derived from <strong className="font-mono text-white bg-teal-500/15 px-1.5 py-0.5 rounded border border-teal-500/25">5 sensory samples</strong>. Real-time averages are
                        computed at each {config.dbStorageInterval} minute index.
                      </span>
                    ) : (
                      <span>
                        Instantaneous telemetry is captured from the final sample at each <strong className="font-mono text-white bg-teal-500/15 px-1.5 py-0.5 rounded border border-teal-500/25">{config.dbStorageInterval} minute</strong>{" "}
                        block, registered without mathematical processing.
                      </span>
                    )}
                  </p>
                </div>
                <div className="flex items-center gap-2 font-mono">
                  <button
                    ...
                    className={`text-xs font-black uppercase py-2 px-3.5 rounded-lg border transition duration-250 cursor-pointer flex items-center gap-1.5 ${
                      sampleBuffer.length === 0 ? "bg-white/5 text-slate-500 border-white/5 cursor-not-allowed" : "bg-teal-500/10 hover:bg-teal-500/20 text-teal-400 border-teal-500/30"
                    }`}
                  >
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span
                        className={`text-xs px-2 py-0.5 rounded-full font-mono font-bold uppercase tracking-wider ${
                          isDbConnected ? "bg-emerald-500/15 text-emerald-400 border border-emerald-500/20" : "bg-amber-500/15 text-amber-300 border border-amber-500/20"
                        }`}
                      >
                        {isDbConnected ? "🟢 CONNECTED (LIVE)" : "🟡 STANDBY / NOT TESTED"}
                      </span>
                    </div>
                    <p className="text-xs text-slate-400 mt-1 leading-normal max-w-[620px]">{isDbConnected ? "" : ""}</p>
                  </div>
                </div>
              ...
              <div className="bg-gradient-to-b from-[#0b1424] to-bg border border-teal-500/30 rounded-2xl max-w-4xl w-full max-h-[90vh] overflow-y-auto shadow-[0_0_50px_rgba(0,240,255,0.15)] flex flex-col pointer-events-auto p-6 space-y-4">
                ...
                <div className="flex items-center justify-between pb-3 border-b border-white/10">
                  <div className="space-y-1">
                    ...
                    <p className="text-xs text-slate-400 font-sans">Konversikan telemetri langsung ke server basis data PostgreSQL Anda.</p>
                  </div>
                  <button onClick={() => setIsIntegratorOpen(false)} className="text-xs font-bold uppercase bg-white/5 hover:bg-white/15 text-slate-300 font-mono py-1.5 px-3 rounded-lg border border-white/10 cursor-pointer transition">
                    ✕ Close
                  </button>
                </div>
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-2 border-b border-white/5">
                  <div className="space-y-1">
                    ...
                    <p className="text-xs text-slate-400 font-sans max-w-[600px]">
                      Aplikasi berjalan di web browser dan mendukung sinkronisasi database <strong>PostgreSQL</strong> melalui file gateway PHP <code className="text-white font-mono bg-white/5 px-1 rounded">api.php</code> di server atau
                      komputer lokal Anda.
                    </p>
                  </div>
                    ...
                    <div className="flex bg-[#050a12] p-0.5 border border-white/10 rounded-lg">
                      <button
                        ...
                        className={`text-[11px] px-2.5 py-1 rounded font-mono uppercase font-black transition cursor-pointer ${
                          dbScriptTab === "sql" ? "bg-teal-500/15 text-teal-300 border border-teal-500/20" : "text-slate-400 hover:text-slate-300"
                        }`}
                      >
                        📜 Manual SQL
                      </button>
                      <button
                        ...
                        className={`text-[11px] px-2.5 py-1 rounded font-mono uppercase font-black transition cursor-pointer ${
                          dbScriptTab === "php" ? "bg-teal-500/15 text-teal-300 border border-teal-500/20" : "text-slate-400 hover:text-slate-300"
                        }`}
                      >
                        ⚡ PHP Installer
                      </button>
                ...
                {dbScriptTab === "sql" ? (
                  <div className="space-y-3">
                    <div className="flex items-center justify-between pt-1">
                      <span className="text-xs font-mono text-slate-300 uppercase font-black">🔌 Metode Manual PostgreSQL:</span>
                      <button
                        onClick={() => {
...
*/`}
                  ...
                  <div className="space-y-4">
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pt-1">
                      <div className="space-y-1">
                        <span className="text-xs font-mono text-teal-400 uppercase font-black block">🔌 Auto-create PHP Code (PostgreSQL via PDO):</span>
...
?>`;
                    ...
                    {dbTestResult.status !== "idle" && (
                      ...
                      >
                        ...
                        {dbTestResult.details && <div className="p-2.5 bg-black/60 rounded border border-white/5 mt-2 text-slate-300 text-xs whitespace-pre-wrap leading-relaxed overflow-x-auto font-mono">{dbTestResult.details}</div>}
                        {dbTestResult.status === "error" && (
                          <div className="mt-3 text-amber-300 text-xs leading-relaxed border-t border-red-500/15 pt-2 font-sans">
                            ...
                            <ul className="list-disc pl-4 mt-1.5 space-y-1 text-slate-300 text-xs">
                              <li>
                                Pastikan server PostgreSQL Anda aktif (default di port <strong className="text-emerald-400">5432</strong>) dan isi database <code className="text-white">db_pelabuhan_telemetry</code> sudah dibuat.
                              </li>
                              <li>
                                Buka file <strong className="text-teal-300 font-mono">php.ini</strong> di XAMPP PHP settings Anda, silakan hilangkan titik koma di awal baris{" "}
                                <code className="text-white bg-black/50 px-1">extension=pdo_pgsql</code> dan <code className="text-white bg-black/50 px-1">extension=pgsql</code> agar PHP XAMPP mendukung driver PostgreSQL, lalu restart
                                Apache.
                              </li>
                              <li>
                                Sesuaikan <strong className="text-teal-400">$username</strong> dan <strong className="text-teal-400">$password</strong> di file <strong className="text-white">api.php</strong> Anda dengan kredensial
                                PostgreSQL Anda.
                              </li>
                              <li>
                                Pastikan web server berjalan di port standar (port 80). Jika menggunakan port custom (misal: 8080), sesuaikan URL anda menjadi{" "}
                                <code className="text-white font-mono text-xs">http://localhost:8080/aws_marine/api.php</code>.
                              </li>
                            </ul>
                          </div>
                        )}
              ...
              <div className="flex items-center gap-4">
                <div
                  className={`w-12 h-12 rounded-xl flex items-center justify-center border transition-all ${
                    showRealDb ? "bg-emerald-500/10 text-emerald-400 border-emerald-500/25 shadow-[0_0_15px_rgba(16,185,129,0.15)]" : "bg-amber-500/10 text-amber-500 border-amber-500/20"
                  }`}
                >
                    ...
                    {filteredLogs.length === 0 ? (
                      ...
                      filteredLogs.map((item, idx) => (
                        <tr key={idx} className="hover:bg-white/5 transition-colors">
                          ...
                          <td className="p-3 text-center border-r border-white/5 text-pink-400 font-bold">{(item.waterPh ?? 7.8).toFixed(2)}</td>
                          ...
                          <td className="p-3 text-center border-r border-white/5 text-amber-400 font-bold">
                            {item.windSpeed.toFixed(1)} / {(item.windSpeed * 1.94384).toFixed(0)}
                          </td>
            ...
            <div className="grid grid-cols-1 xl:grid-cols-12 gap-6 items-start">
              ...
              <div className="xl:col-span-4 space-y-6">
                <div className="bg-gradient-to-b from-[#0b1424] to-bg border border-white/10 rounded-2xl p-5 space-y-4 shadow-xl">
                  <div className="text-xs md:text-sm uppercase font-bold text-[#f59e0b] tracking-[0.2em] border-b border-white/5 pb-2">🔒 Hardware & Network Config</div>
                  ...
                  <div className="space-y-3 font-sans">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        <label className="text-xs md:text-xs uppercase font-bold text-teal-400 font-mono tracking-wider block mb-1.5">{config.transport === "SERIAL" ? "COM Port" : "Moxa IP / Host"}</label>
                        <input
                          type="text"
                      ...
                      <div>
                        <label className="text-xs md:text-xs uppercase font-bold text-teal-400 font-mono tracking-wider block mb-1.5">{config.transport === "SERIAL" ? "Baudrate" : "Socket Port"}</label>
                        <input
                          type="text"
                    ...
                    {config.transport === "MOXA_TCP" && (
                      <div className="p-3.5 bg-slate-950/90 border border-white/10 rounded-xl space-y-3 font-mono">
                        ...
                        <div className="text-xs space-y-1 bg-black/40 p-2.5 rounded border border-white/5 text-slate-300">
                          ...
                          {moxaStatus?.error && <div className="mt-1.5 text-[10px] text-red-300 bg-red-500/10 p-1.5 rounded border border-red-500/15">⚠️ Msg: {moxaStatus.error}</div>}
                          {isLiveActive && moxaStatus && !moxaStatus.connected && (
                            <div className="mt-2 text-[10px] text-emerald-300 bg-emerald-500/10 p-2 rounded border border-emerald-500/20 leading-relaxed font-sans">
                              ℹ️ <strong>Status Aliran Data: AKTIF</strong>. Meskipun daemon melaporkan status TCP terputus atau mencoba menghubungkan kembali (kemungkinan karena Moxa membatasi sesi koneksi tunggal atau ada daemon ganda),
                              dashboard Anda <strong>tetap berhasil menerima data real-time secara berkala</strong>.
                            </div>
                          )}
                        </div>
                        ...
                        {window.location.protocol === "https:" && (
                          <div className="p-2.5 text-[10px] text-amber-300 bg-amber-500/10 rounded border border-amber-500/15 leading-relaxed space-y-1">
                            ...
                            <p>
                              Dashboard saat ini dibuka via <strong>HTTPS</strong>, sehingga browser modern otomatis <strong>memblokir</strong> sambungan langsung ke daemon lokal yang berjalan via{" "}
                              <strong>HTTP (CORS / Mixed Content)</strong>.
                            </p>
                            <p className="font-bold text-white">Agar data masuk ke dashboard, Anda harus:</p>
                            <ul className="list-disc pl-3.5 space-y-0.5">
                              <li>
                                Buka Dashboard secara lokal menggunakan protokol HTTP standar (misal: <strong>http://localhost:3000</strong>).
                              </li>
                              <li>
                                Atau klik icon gembok di sebelah kiri address bar browser Anda, pilih <strong>Site Settings</strong>, lalu set opsi <strong>Insecure Content</strong> menjadi <strong>Allow / Izinkan</strong>.
                              </li>
                            </ul>
                          </div>
                        )}
                    ...
                    <div className="border-t border-white/5 pt-3">
                      <label className="text-xs uppercase font-bold text-rose-400 tracking-wider block mb-1.5 flex items-center gap-1.5">🔒 Device Offline: Dashboard Hiden</label>
                      <select
                    ...
                    <div className="border-t border-white/5 pt-3 space-y-3">
                      <div>
                        ...
                        <select value={config.cloudMode} onChange={(e) => setConfig({ ...config, cloudMode: e.target.value })} className="w-full bg-[#050a12] border border-[#22c55e]/25 font-mono text-xs p-2 text-white rounded outline-none">
                          <option value="OFF">OFF</option>
                    ...
                    <div className="border-t border-white/5 pt-3 space-y-2">
                      <label className="text-xs uppercase font-bold text-pink-400 tracking-wider block font-mono mb-1">🧪 Water pH Alarm Limits</label>
                      <div className="grid grid-cols-2 gap-3">
                        <div>
                    ...
                    <div className="border-t border-white/5 pt-3 space-y-2">
                      <label className="text-xs uppercase font-bold text-sky-400 tracking-wider block font-mono mb-1">🌧️ Rainfall Alarm Settings</label>
                      <div>
                    ...
                    <div className="border-t border-white/5 pt-3 space-y-3">
                      <label className="text-xs uppercase font-bold text-amber-400 tracking-wider block font-mono mb-1">🌬️ Wind Warning & Alarm Threshold Settings</label>
                      <div className="grid grid-cols-2 gap-3">
                        <div>
                        ...
                        <div className="flex justify-between items-center bg-[#050a12] p-2.5 rounded-lg border border-white/10">
                          ...
                          <button
                            ...
                            className={`px-3 py-1 text-[10px] font-mono font-bold uppercase rounded-lg border transition-all cursor-pointer ${
                              isAudioAlarmEnabled ? "bg-emerald-500/20 text-emerald-400 border-emerald-500/40" : "bg-slate-800 text-slate-500 border-white/10"
                            }`}
                          >
                    ...
                    <div className="border-t border-white/5 pt-3 space-y-2">
                      <label className="text-xs uppercase font-bold text-teal-400 tracking-wider block">📁 Database Archiving & Storage Settings</label>
                      <div className="space-y-3">
                        <div>
                        ...
                        <div>
                          <span className="text-xs text-slate-400 uppercase font-mono block mb-1">
                            Averaging & Logging Interval:{" "}
                            {config.dbStorageInterval === 0 ? (
                              <span className="text-emerald-400 font-extrabold animate-pulse">⚡ INSTANT / SETIAP DATA MASUK</span>
                            ) : (
                              <span className="text-white font-bold">{config.dbStorageInterval || 10} Menit</span>
                            )}
                          </span>
                          ...
                          {config.dbStorageInterval === 0 ? (
                            ...
                            <p className="text-[10px] text-slate-500 font-sans mt-1.5 leading-normal">Standard WMO: 10 menit (data di-buffer dan dirata-rata terlebih dahulu sebelum masuk database).</p>
                          )}
                        </div>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="flex border border-white/10 p-0.5 bg-black/40 rounded-xl">
                      <button
                        ...
                        className={`flex-1 py-1.5 text-[10px] font-bold uppercase rounded-lg text-center transition-all cursor-pointer ${
                          adminModalMode === "login" ? "bg-[#00f0ff]/15 text-[#00f0ff] border border-[#00f0ff]/30" : "text-slate-400 hover:text-white"
                        }`}
                      >
                        Ganti Password
                      </button>
                      <button
                        ...
                        className={`flex-1 py-1.5 text-[10px] font-bold uppercase rounded-lg text-center transition-all cursor-pointer ${
                          adminModalMode === "register" ? "bg-emerald-500/15 text-emerald-400 border border-emerald-500/30" : "text-slate-400 hover:text-white"
                        }`}
                      >
                    ...
                    {adminModalMode === "login" ? (
                      <form onSubmit={handleChangePasscodeSubmit} className="space-y-3">
                        ...
                        {changePassError && <p className="text-[10px] font-mono text-rose-400 bg-rose-500/10 border border-rose-500/20 p-2 rounded text-center">⚠️ {changePassError}</p>}

                        {changePassSuccess && <p className="text-[10px] font-mono text-emerald-400 bg-emerald-500/10 border border-emerald-500/20 p-2 rounded text-center font-bold">✅ {changePassSuccess}</p>}

                        <button type="submit" className="w-full bg-[#00f0ff] hover:bg-[#00d0e0] text-black font-black py-2.5 rounded-xl text-[10px] uppercase tracking-wider transition-all cursor-pointer font-sans">
                      ...
                      <form onSubmit={handleRegisterAdmin} className="space-y-3">
                        ...
                        {registerError && <p className="text-[10px] font-mono text-rose-400 bg-rose-500/10 border border-rose-500/20 p-2 rounded text-center">⚠️ {registerError}</p>}

                        <button type="submit" className="w-full bg-emerald-500 hover:bg-emerald-600 text-white font-black py-2.5 rounded-xl text-[10px] uppercase tracking-wider transition-all cursor-pointer font-sans">
              ...
              <div className="xl:col-span-8 space-y-6">
                <div className="grid grid-cols-2 gap-4">
                  <div className="bg-gradient-to-b from-[#0b1424] to-bg p-4.5 rounded-xl border border-white/10 flex justify-between items-center">
                    <div>
                      ...
                      <select value={config.ind_date} onChange={(e) => setConfig({ ...config, ind_date: e.target.value })} className="bg-[#050a12] border border-white/10 font-mono text-xs p-1 text-white rounded outline-none">
                        {[...Array(20)].map((_, i) => (
                    ...
                    <div className="bg-[#050a12] border border-yellow-500 rounded p-2 text-center text-xs font-mono font-black text-yellow-500 min-w-[70px]">{format(new Date(), "dd-MM-yyyy")}</div>
                  </div>

                  <div className="bg-gradient-to-b from-[#0b1424] to-bg p-4.5 rounded-xl border border-white/10 flex justify-between items-center">
                    <div>
                      <span className="text-xs uppercase tracking-wider text-[#00f0ff] font-bold block mb-1">Station ID Index</span>
                      <select value={config.ind_id} onChange={(e) => setConfig({ ...config, ind_id: e.target.value })} className="bg-[#050a12] border border-white/10 font-mono text-xs p-1 text-white rounded outline-none">
                        {[...Array(20)].map((_, i) => (
                          <option key={i} value={i}>
                            {i}
                          </option>
                    ...
                    <div className="bg-[#050a12] border border-[#00f0ff] rounded p-2 text-center text-xs font-mono font-black text-[#00f0ff] min-w-[70px]">{config.idStation}</div>
                  </div>
                </div>
                ...
                <div className="bg-gradient-to-br from-[#0c1625] to-bg border border-white/10 p-6 rounded-3xl">
                  <div className="text-xs font-bold text-[#00f0ff] uppercase tracking-widest mb-6 pb-2 border-b border-white/5">🕹️ Sensor Channel Mapping Indexes</div>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    {[
                      ...
                      { label: "Water pH", key: "ch_18", color: "#f5d0fe", source: (currentData.waterPh ?? 7.8).toFixed(2) },
                      ...
                      <div key={s_idx} className="bg-[#050a12]/70 border border-white/5 p-3 rounded-lg flex flex-col justify-between gap-1">
                        <span className="text-xs uppercase font-mono tracking-wider font-extrabold text-slate-400 block">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <button onClick={() => handleSaveConfig(config)} className="bg-emerald-500 hover:bg-emerald-600 font-mono rounded-lg p-3 text-xs font-black text-slate-950 uppercase cursor-pointer text-center">
                    SAVE ACTIVE CONFIG
                  </button>
          ...
          <div className="space-y-6 animate-fade-in pb-10">
            ...
            <div className="bg-gradient-to-r from-[#0b1424] via-[#040912] to-bg p-6 rounded-3xl border border-[#00f0ff]/20 flex flex-col md:flex-row gap-5 justify-between items-start md:items-center shadow-xl relative overflow-hidden">
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  <div className="flex items-center gap-2">
                    <span className="text-[10px] bg-emerald-500/30 text-emerald-300 font-bold px-2 py-0.5 rounded border border-emerald-500/30 uppercase tracking-widest font-mono">Official Marine Database Sync</span>
                    <span className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse" />
                  </div>
                  <h2 className="text-xl md:text-2xl font-black text-white tracking-tight uppercase mt-1">Prakiraan Cuaca Maritim BMKG</h2>
                  <div className="flex flex-col gap-0.5">
                    <p className="text-xs text-slate-400 uppercase tracking-wider font-semibold font-mono">Lokasi: {config.bmkgPortLabel || "Pelabuhan Ciwandan"} (BMKG Maritim)</p>
              ...
              <div className="flex flex-wrap items-center gap-3 w-full md:w-auto">
                <button
                  ...
                  className={`px-4 py-2.5 rounded-xl text-xs font-mono tracking-wider transition-all uppercase flex items-center gap-2 border cursor-pointer ${
                    isLoadingBmkg ? "bg-emerald-950/20 text-slate-500 border-white/5 cursor-not-allowed" : "bg-emerald-500/15 text-emerald-400 border-emerald-500/30 hover:bg-emerald-500/25 active:bg-emerald-500/30 font-black"
                  }`}
                >
                ...
                <div className="flex bg-[#03070f] rounded-xl p-1 border border-white/10 font-mono font-bold">
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all uppercase cursor-pointer ${
                      bmkgLayout === "table" ? "bg-emerald-500/15 text-emerald-400 shadow-[0_0_10px_rgba(16,185,129,0.1)]" : "text-slate-500 hover:text-slate-300"
                    }`}
                  >
                    Table
                  </button>
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all uppercase cursor-pointer ${
                      bmkgLayout === "cards" ? "bg-emerald-500/15 text-emerald-400 shadow-[0_0_10px_rgba(16,185,129,0.1)]" : "text-slate-500 hover:text-slate-300"
                    }`}
                  >
                    Grid
                  </button>
                      ...
                      return (
                        <button
                          ...
                          className={`px-3 py-1.5 text-[10px] font-mono tracking-tight font-extrabold rounded-lg whitespace-nowrap border shrink-0 transition-all cursor-pointer ${
                            isSelected ? "bg-emerald-500/10 border-emerald-400 text-emerald-400 font-black shadow-[0_0_8px_rgba(16,185,129,0.25)]" : "bg-[#03070f] border-white/5 text-slate-400 hover:text-white hover:border-white/10"
                          }`}
                        >
                          📍 {item.label}
                        </button>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
              ...
              <div className="bg-gradient-to-b from-[#0b1424] to-[#010610] border border-white/5 p-5 rounded-2xl flex flex-col justify-between">
                ...
                <div className="mt-4 pt-3 border-t border-white/5 flex justify-between items-center text-xs">
                  ...
                  <span className="text-pink-400 font-bold uppercase font-mono">{selectedBmkgRow ? (selectedBmkgRow.pasut > 1.0 ? "PASANG TINGGI" : "PASANG NORMAL") : "PASANG NORMAL"}</span>
                </div>
              </div>
              ...
              <div className="bg-gradient-to-b from-[#0b1424] to-[#010610] border border-white/5 p-5 rounded-2xl flex flex-col justify-between">
                ...
                <div className="mt-4 pt-3 border-t border-white/5 flex justify-between items-center text-xs">
                  ...
                  <span className="text-emerald-400 font-black uppercase font-mono">{selectedBmkgRow && selectedBmkgRow.visibility >= 8 ? "SANGAT CLEAR" : "SEDANG/TERBATAS"}</span>
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
              ...
              <div className="lg:col-span-8 space-y-4">
                ...
                {(() => {
                  const filtered = bmkgForecast.filter((item) => {
                    ...
                    return item.waktu.toLowerCase().includes(search) || item.cuaca.toLowerCase().includes(search) || item.anginDir.toLowerCase().includes(search) || item.gelombangKet.toLowerCase().includes(search);
                  });
                  ...
                  if (filtered.length === 0) {
                    return (
                      <div className="bg-[#0b1424]/40 border border-white/5 py-12 px-6 rounded-3xl text-center">
                        ...
                        <button onClick={() => setBmkgSearchText("")} className="mt-3 text-emerald-400 font-bold text-xs uppercase underline tracking-wider cursor-pointer">
                          Clear Search
                        </button>
                      </div>
                    );
                                ...
                                return (
                                  ...
                                  >
                                    ...
                                    <td className="p-4 text-center font-mono">
                                      ...
                                      <div className="text-[10px] text-amber-500 font-extrabold">
                                        {row.anginSpeed} kt <span className="opacity-50">gust {row.anginGust}</span>
                                      </div>
                                    </td>
                                    ...
                                    <td className="p-4 text-center">
                                      ...
                                      <span
                                        className={`inline-block text-[9px] font-black px-2 py-0.5 rounded uppercase ${
                                          row.gelombangKet === "Tenang" ? "bg-sky-500/10 text-sky-400 border border-sky-500/20" : "bg-emerald-500/10 text-emerald-400 border border-emerald-500/20"
                                        }`}
                                      >
                                        {row.gelombangKet}
                                      </span>
                                    ...
                                    {/* Column 6: Visibility */}
                                    <td className="p-4 text-center font-mono text-slate-300">{row.visibility} km</td>

                                    {/* Column 7: Temperature */}
                                    <td className="p-4 text-center font-mono font-black text-[#f59e0b]">{row.suhu}°C</td>

                                    {/* Column 8: Humidity */}
                                    <td className="p-4 text-center font-mono text-sky-400">{row.kelembaban}%</td>

                                    {/* Column 9: Tide elevation */}
                                    <td className="p-4 text-center font-mono font-black text-pink-400">+{row.pasut.toFixed(2)} m</td>
                                  </tr>
                                );
                              })}
                            </tbody>
                        ...
                        {filtered.map((row, idx) => {
                          ...
                          return (
                            <div
                              key={idx}
                              onClick={() => setSelectedForecastIndex(originalIndex)}
                              className={`bg-gradient-to-b from-[#0b1424] to-bg border p-5 rounded-2.5xl cursor-pointer transition-all ${
                                isSelected ? "border-emerald-500/60 bg-emerald-500/5 shadow-[0_0_20px_rgba(16,185,129,0.08)]" : "border-white/5"
                              }`}
                            >
                              ...
                              <div className="grid grid-cols-3 gap-2 mt-3 font-sans">
                                <div>
                                  ...
                                  <span className="text-[11px] text-amber-500 font-mono font-bold leading-tight">
                                    {row.anginDir}
                                    <br />
                                    {row.anginSpeed} kt
                                  </span>
                                ...
                                <div>
                                  ...
                                  <span className="text-[11px] text-teal-400 font-mono font-bold leading-tight">
                                    {row.arusDir}
                                    <br />
                                    {row.arusSpeed} kt
                                  </span>
                                ...
                                <div>
                                  ...
                                  <span className="text-[11px] text-[#f59e0b] font-mono font-bold leading-tight">
                                    {row.suhu}°C
                                    <br />
                                    {row.kelembaban}%
                                  </span>
              ...
              <div className="lg:col-span-4">
                {selectedBmkgRow ? (
                    ...
                    return (
                      <div className="bg-gradient-to-b from-[#0b1424] via-[#020710] to-bg border border-emerald-500/20 p-6 rounded-3xl space-y-5 shadow-2xl relative overflow-hidden">
                        ...
                        <div className="pb-3 border-b border-white/5">
                          <span className="text-[10px] font-bold text-emerald-400 uppercase tracking-widest font-mono">Selected Hour Inspector</span>
                        ...
                        <div className="bg-[#030811] border border-white/5 p-4 rounded-2xl relative overflow-hidden">
                          <span className="text-[10px] text-slate-500 uppercase font-black block mb-3 font-sans">Arah Arus Air Laut (Ocean Current)</span>
                          <div className="flex items-center justify-between">
                            <div className="flex items-center gap-2.5">
                        ...
                        <div className="bg-[#030811] border border-white/5 p-4 rounded-2xl">
                          <span className="text-[10px] text-slate-500 uppercase font-black block mb-3 font-sans">Kondisi Angin & Hembusan (Wind Profile)</span>
                          <div className="flex items-center justify-between">
                            <div className="flex items-center gap-2.5">
                        ...
                        <div className="bg-[#041e12] border border-emerald-500/20 p-4 rounded-2xl font-sans">
                          <span className="text-[10px] font-black text-emerald-400 uppercase tracking-wider block mb-1">Port Commander Safety Assessment</span>
                          <p className="text-xs text-emerald-200/90 leading-relaxed font-semibold">
                            Ketinggian Gelombang Laut ({selected.gelombangVal}m - {selected.gelombangKet}) dan kecepatan hembusan angin ({selected.anginSpeed} knot) berada pada ambang batas aman. Operasional bongkar muat & penyandaran kapal
                            di {config.bmkgPortLabel || "Pelabuhan Ciwandan"} dapat dilaksanakan secara normal.
                          </p>
                        </div>
                      </div>
                    );
                  ...
                  <div className="bg-[#0b1424]/40 border border-white/5 p-6 rounded-3xl text-center font-sans text-slate-500 py-20">Klik baris perkiraan di sebelah kiri untuk melihat rincian instrumen di panel monitor ini.</div>
                )}
              </div>
            </div>
          </div>
          ...
          <div className="space-y-6 animate-fade-in pb-10">
            ...
            <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
              ...
              <div className="bg-[#0b1424]/90 border border-white/10 rounded-3xl p-6 space-y-4 shadow-xl relative">
                ...
                <div className="flex justify-between items-center pb-2 border-b border-white/5">
                  <span className="text-sm font-black text-[#0ea5e9] tracking-wider font-sans uppercase flex items-center gap-2">🌊 1. Water Level & Pasang Surut / Tide (Meter)</span>
                  <div className="flex items-center gap-3">
                ...
                <div className="h-[280px]">
                  <ResponsiveContainer width="100%" height="100%">
                    <AreaChart
                      data={chartData.slice(-144).map((item) => {
                        return {
                          ...item,
                          seaLevelM: parseFloat((item.seaLevel / 100).toFixed(3)),
              ...
              <div className="bg-[#0b1424]/90 border border-white/10 rounded-3xl p-6 space-y-4 shadow-xl relative">
                ...
                <div className="flex justify-between items-center pb-2 border-b border-white/5">
                  <span className="text-sm font-black text-[#f59e0b] tracking-wider font-sans uppercase flex items-center gap-2">🌡️ 2. Temperature Trends (Udara vs Air Laut)</span>
                  <div className="flex items-center gap-3">
              ...
              <div className="bg-[#0b1424]/90 border border-white/10 rounded-3xl p-6 space-y-4 shadow-xl relative">
                ...
                <div className="flex justify-between items-center pb-2 border-b border-white/5">
                  <span className="text-sm font-black text-[#22d3ee] tracking-wider font-sans uppercase flex items-center gap-2">🌀 3. Barometric Pressure (hPa)</span>
                ...
                <div className="h-[280px]">
                  <ResponsiveContainer width="100%" height="100%">
                    <AreaChart data={chartData.slice(-144)}>
                      <defs>
              ...
              <div className="bg-[#0b1424]/90 border border-white/10 rounded-3xl p-6 space-y-4 shadow-xl relative">
                ...
                <div className="flex justify-between items-center pb-2 border-b border-white/5">
                  <span className="text-sm font-black text-[#fc63a3] tracking-wider font-sans uppercase flex items-center gap-2">🧪 4. Water pH Quality Index</span>
              ...
              <div className="bg-[#0b1424]/90 border border-white/10 rounded-3xl p-6 space-y-4 shadow-xl relative">
                ...
                <div className="flex justify-between items-center pb-2 border-b border-white/5">
                  <span className="text-sm font-black text-[#f97316] tracking-wider font-sans uppercase flex items-center gap-2">⚡ 5. Wind Gust Speeds (Knot)</span>
              ...
              <div className="bg-[#0b1424]/90 border border-white/10 rounded-3xl p-6 space-y-4 shadow-xl relative">
                <div className="absolute top-0 left-0 w-3 h-3 border-t border-l border-[#f59e0b]/50 rounded-tl-xl" />
                <div className="flex justify-between items-center pb-2 border-b border-white/5">
                  <span className="text-sm font-black text-[#f59e0b] tracking-wider font-sans uppercase flex items-center gap-2">☀️ 7. Solar Radiation / Radiasi Matahari (W/m²)</span>
                ...
                <div className="h-[280px]">
                  <ResponsiveContainer width="100%" height="100%">
                    <AreaChart data={chartData.slice(-144)} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                      <defs>
              ...
              <div className="bg-[#0b1424]/90 border border-white/10 rounded-3xl p-6 space-y-4 shadow-xl relative">
                ...
                <div className="flex justify-between items-center pb-2 border-b border-white/5">
                  <span className="text-sm font-black text-[#10b981] tracking-wider font-sans uppercase flex items-center gap-2">🔋 8. System Battery Voltage / Tegangan Baterai Logger (V)</span>
              ...
              <div className="xl:col-span-2 bg-[#0b1424]/90 border border-white/10 rounded-3xl p-6 space-y-4 shadow-xl relative">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between pb-2 border-b border-white/5 gap-2">
                  <div className="space-y-0.5">
                    <span className="text-sm font-black text-[#0ea5e9] tracking-wider font-sans uppercase flex items-center gap-2">🌧️ 6. Rainfall Radar Intensity & 24H Cumulative Track</span>
                    <p className="text-[10px] text-slate-400 font-sans">
                      Warna tracker menunjukkan intensitas hujan: <span className="text-[#22c55e] font-black">Hijau (Ringan)</span>, <span className="text-[#eab308] font-black">Kuning (Sedang)</span>,{" "}
                      <span className="text-[#ef4444] font-black">Merah (Lebat &gt;= {parseFloat(config.rainWarningThreshold || "10.0")} mm)</span>
                    </p>
                  </div>
                  ...
                  <div className="lg:col-span-4 bg-[#050a12]/80 border border-white/5 p-4 rounded-2xl flex flex-col justify-between space-y-3">
                    ...
                    <div className="grid grid-cols-12 gap-1.5 flex-1 content-center py-2">
                      {history.slice(-144).map((item, idx, arr) => {
                        ...
                        return <div key={idx} title={`${format(item.timestamp, "HH:mm")} - ${titleText}`} className={`h-4 rounded cursor-help transition-all hover:scale-125 hover:z-10 ${color} ${ring}`} />;
                      })}
                    </div>
            ...
            <div className="bg-[#0b1424] border border-white/10 p-5 rounded-3xl flex flex-col md:flex-row gap-4 justify-between items-center bg-gradient-to-r from-emerald-950/15 to-transparent">
              <p className="text-xs text-slate-400 font-sans leading-relaxed text-center md:text-left">
                ⚠️ <strong>INFORMASI OPERASIONAL AWS:</strong> Grafik di atas menyajikan feed intermitten 24 jam terakhir dari stasiun meteorologi fisik <strong>Automatic Weather Station</strong>. Gunakan data telemetry ini sebagai acuan
                validasi operasional yang presisi.
              </p>
              <span className="text-[10px] font-mono text-emerald-400 font-extrabold bg-emerald-500/10 px-3 py-1.5 rounded-xl border border-emerald-500/20 whitespace-nowrap uppercase tracking-widest">AWS ONLINE SECURE</span>
            </div>
          </div>
        )}
      </main>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/90 backdrop-blur-md animate-fade-in font-sans">
          <div className="bg-[#07111e] border-2 border-[#00f0ff]/30 w-full max-w-6xl h-[85vh] rounded-3xl shadow-[0_0_60px_rgba(0,240,255,0.3)] flex flex-col overflow-hidden relative text-slate-100">
            ...
            <div className="flex-1 flex flex-col md:flex-row overflow-hidden">
              ...
              <div className="w-full md:w-64 bg-[#050b14] border-r border-white/5 p-4 flex flex-col gap-1.5 overflow-y-auto shrink-0">
                ...
                <button
                  ...
                  className={`w-full text-left py-2.5 px-3 rounded-lg text-xs font-bold transition-all border flex items-center gap-2.5 ${
                    activeManualChapter === "intro" ? "bg-[#00f0ff]/10 text-[#00f0ff] border-[#00f0ff]/30" : "text-slate-400 border-transparent hover:text-white hover:bg-white/5"
                  }`}
                >
                ...
                <button
                  ...
                  className={`w-full text-left py-2.5 px-3 rounded-lg text-xs font-bold transition-all border flex items-center gap-2.5 ${
                    activeManualChapter === "realtime" ? "bg-[#00f0ff]/10 text-[#00f0ff] border-[#00f0ff]/30" : "text-slate-400 border-transparent hover:text-white hover:bg-white/5"
                  }`}
                >
                ...
                <button
                  ...
                  className={`w-full text-left py-2.5 px-3 rounded-lg text-xs font-bold transition-all border flex items-center gap-2.5 ${
                    activeManualChapter === "option" ? "bg-[#00f0ff]/10 text-[#00f0ff] border-[#00f0ff]/30" : "text-slate-400 border-transparent hover:text-white hover:bg-white/5"
                  }`}
                >
                ...
                <button
                  ...
                  className={`w-full text-left py-2.5 px-3 rounded-lg text-xs font-bold transition-all border flex items-center gap-2.5 ${
                    activeManualChapter === "database" ? "bg-[#00f0ff]/10 text-[#00f0ff] border-[#00f0ff]/30" : "text-slate-400 border-transparent hover:text-white hover:bg-white/5"
                  }`}
                >
                ...
                <button
                  ...
                  className={`w-full text-left py-2.5 px-3 rounded-lg text-xs font-bold transition-all border flex items-center gap-2.5 ${
                    activeManualChapter === "bmkg" ? "bg-[#00f0ff]/10 text-[#00f0ff] border-[#00f0ff]/30" : "text-slate-400 border-transparent hover:text-white hover:bg-white/5"
                  }`}
                >
                ...
                <button
                  ...
                  className={`w-full text-left py-2.5 px-3 rounded-lg text-xs font-bold transition-all border flex items-center gap-2.5 ${
                    activeManualChapter === "troubleshoot" ? "bg-[#00f0ff]/10 text-[#00f0ff] border-[#00f0ff]/30" : "text-slate-400 border-transparent hover:text-white hover:bg-white/5"
                  }`}
                >
              ...
              <div className="flex-1 p-6 md:p-8 overflow-y-auto bg-gradient-to-b from-[#07111e] to-[#040a12] space-y-6">
                  ...
                  <div className="space-y-6">
                    <div className="bg-gradient-to-r from-blue-950/40 to-transparent p-5 rounded-2xl border border-blue-500/20">
                      ...
                      <p className="text-xs text-slate-300 leading-relaxed mb-3">
                        ...
                        real-time. Sistem ini menghubungkan sensor-sensor lapangan melalui hardware gateway <strong>Moxa Router (TCP/IP atau Serial RS232)</strong> dan mengamankan perekaman log data ke dalam engine database terintegrasi
                        secara otomatis.
                      </p>
                      <div className="bg-cyan-500/10 border border-cyan-500/30 rounded-xl p-3.5 space-y-1">
                        ...
                        <p className="text-[11px] text-slate-300 leading-relaxed">
                          Sistem aplikasi ini sepenuhnya <strong>berbasis web (web-based client-server)</strong>, yang berarti aplikasi dapat dipanggil, diakses, dan dimonitor secara instan{" "}
                          <strong>di mana saja dari komputer, laptop, tablet, atau smartphone lain dalam 1 jaringan (LAN/Wi-Fi)</strong> yang sama. Anda cukup membuka browser dan mengetik alamat IP host server monitoring pelabuhan tanpa
                          perlu melakukan instalasi software tambahan di setiap perangkat klien.
                        </p>
                      </div>
                    </div>
                        ...
                        <div className="grid grid-cols-12 gap-3 aspect-[1.8/1] text-[8px] font-mono text-slate-400">
                          ...
                          <div className="col-span-3 bg-[#0b1424] border border-white/5 rounded-lg p-2 flex flex-col justify-between">
                            ...
                            <div className="mt-1">
                              ...
                              <div className="space-y-0.5">
                                <div className="flex justify-between bg-white/5 p-0.5 rounded">
                                  <span>Hum:</span>
                                  <span className="text-white">67 %</span>
                                </div>
                                <div className="flex justify-between bg-white/5 p-0.5 rounded">
                                  <span>Rad:</span>
                                  <span className="text-amber-400">116 W/m²</span>
                                </div>
                              </div>
                            </div>
                            <div className="mt-1">
                              ...
                              <div className="flex justify-between bg-white/5 p-0.5 rounded items-center">
                                <span>pH:</span>
                                <span className="text-pink-400 font-bold">7.64 IDEAL</span>
                              </div>
                            </div>
                          </div>
                          ...
                          <div className="col-span-6 bg-[#0b1424] border border-white/5 rounded-lg p-2 flex flex-col items-center justify-between">
                            ...
                            <div className="w-20 h-20 rounded-full border-2 border-dashed border-[#00f0ff]/20 flex items-center justify-center relative my-1">
                              ...
                              <div className="absolute text-center bg-slate-950 px-1.5 py-0.5 rounded border border-white/10 text-[9px] text-[#00ff66] font-bold">4.2 m/s</div>
                            </div>
                          ...
                          <div className="col-span-3 bg-[#0b1424] border border-white/5 rounded-lg p-2 flex flex-col justify-between">
                            <div>
                              ...
                              <div className="space-y-0.5">
                                <div className="bg-white/5 p-0.5 rounded flex justify-between">
                                  <span>Max Spd:</span>
                                  <span className="text-white">14.7 m/s</span>
                                </div>
                                <div className="bg-white/5 p-0.5 rounded flex justify-between">
                                  <span>Min Spd:</span>
                                  <span className="text-white">2.0 m/s</span>
                                </div>
                              </div>
                            </div>
                            <div className="mt-1">
                              ...
                              <div className="space-y-0.5 text-[7px]">
                                <div className="flex justify-between border-b border-white/5">
                                  <span>14:47</span>
                                  <span className="text-white">W 9.6 m/s</span>
                                </div>
                                <div className="flex justify-between border-b border-white/5">
                                  <span>14:47</span>
                                  <span className="text-white">N 8.2 m/s</span>
                                </div>
                                <div className="flex justify-between border-b border-white/5">
                                  <span>14:47</span>
                                  <span className="text-white">E 4.2 m/s</span>
                                </div>
                              </div>
                            </div>
                          </div>
                        </div>
                        <p className="text-[10px] text-slate-400 italic">* Tampilan skema dashboard realtime di atas mencakup seluruh widget pembacaan live sesuai screenshot AWS OS Connection.</p>
                      </div>
                    </div>
                  </div>
                )}
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="bg-[#0b1424] p-5 rounded-2xl border border-blue-500/20 space-y-3">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-[11px] text-slate-400">
                          <div className="space-y-1.5">
                            ...
                            Sudut dermaga dikonfigurasi melalui menu <span className="text-purple-400">Option</span> (misal: <em>45° Clockwise</em>). Kompas secara dinamis memutar garis lintang hijau yang berlabel{" "}
                            <strong className="text-emerald-400">STARBOARD (Kiri Layar)</strong> dan <strong className="text-rose-400">PORT (Kanan Layar)</strong>. Ini mensimulasikan orientasi lambung kapal saat sedang sandar sejajar
                            dermaga.
                          </div>
                          <div className="space-y-1.5">
                            <strong className="text-white block">2. Penghitungan Relative Wind Angle</strong>
                            Sistem secara real-time menghitung <strong>Relative Wind (Suhu/Sudut Angin Relatif)</strong> terhadap badan kapal (misal: <em>41° Azimuth</em>). Ini sangat krusial karena memberi tahu petugas pandu pelabuhan dari
                            sudut sebelah mana angin menghantam dinding kapal saat proses bersandar.
                          </div>
                        </div>
                        <div className="bg-slate-950 p-3 rounded-lg border border-white/5 text-[11px] text-amber-300">
                          <strong>⚠️ Manfaat Operasional Utama:</strong> Mencegah bahaya kecelakaan kapal membentur beton dermaga (docking collision). Angin samping kencang (crosswind) dari arah laut lepas sangat berbahaya; dengan widget
                          ini, kapten kapal dan operator pelabuhan dapat langsung mengantisipasi gaya dorong samping angin pada struktur kapal secara instan.
                        </div>
                      </div>
                      ...
                      <div className="bg-[#0b1424] p-5 rounded-2xl border border-yellow-500/20 space-y-3">
                        ...
                        <p className="text-[11px] text-slate-300 leading-relaxed">Parameter peringatan kecepatan angin dikelola langsung melalui indikator status di bagian kanan bawah dashboard realtime.</p>
                        <ul className="text-[11px] text-slate-300 space-y-2 list-disc pl-5">
                          <li>
                            <strong>Live Wind Speed Limits:</strong> Widget ini memantau ambang batas kecepatan angin riil. Terbagi menjadi <strong className="text-cyan-400">WIND MAX</strong> (kecepatan puncak tiupan angin aktual) dan{" "}
                            <strong className="text-cyan-400">WIND MIN</strong> (kecepatan hembusan terendah) dalam rentang sampling berjalan.
                          </li>
                          <li>
                            <strong>Last Gust Occurrence Event:</strong> Angin kencang mendadak (Gust) adalah ancaman terbesar bagi kestabilan kapal dan bongkar muat kontainer. Sistem mencatat secara otomatis kapan terjadinya lonjakan
                            hembusan angin ekstrem (kecepatan Gust dan pencatatan Waktu Kejadian / Time of Gust).
                          </li>
                          <li>
                            <strong>Mengapa Warning Angin Ini Sangat Penting?</strong>
                            <ul className="list-circle pl-5 mt-1 space-y-1 text-slate-400">
                              <li>
                                <strong>Batas Aman Crane:</strong> Alat bongkar muat pelabuhan (Gantry Crane) wajib menghentikan operasional jika hembusan angin melebihi 15 m/s karena berisiko roboh atau mematahkan boom penopang.
                              </li>
                              <li>
                                <strong>Kestabilan Mooring:</strong> Tali penambat kapal di dermaga bisa putus jika angin kencang berdurasi lama menghantam lambung kapal secara tegak lurus.
                              </li>
                            </ul>
                          </li>
                        </ul>
                    ...
                    <div className="border-t border-white/5 pt-4">
                      ...
                      <div className="border border-white/10 rounded-2xl bg-slate-950/60 p-4 space-y-3 shadow-inner">
                          ...
                          <div className="col-span-8 bg-[#0b1424] border border-white/5 rounded-lg p-2 space-y-2">
                            ...
                            <div className="grid grid-cols-3 gap-2 text-center text-[7px]">
                              <div className="bg-slate-950/50 p-1 rounded">
                                <span>Air Temp:</span> <strong className="text-white">28.5 °C</strong>
                              </div>
                              <div className="bg-slate-950/50 p-1 rounded">
                                <span>Humidity:</span> <strong className="text-white">67 %</strong>
                              </div>
                              <div className="bg-slate-950/50 p-1 rounded">
                                <span>Solar Rad:</span> <strong className="text-white">433 W/m²</strong>
                              </div>
                            </div>
                          </div>
                        ...
                        <p className="text-[10px] text-slate-400 italic">* Tampilan skema chart tren cuaca dan wind rose di atas mencakup seluruh widget sesuai screenshot Analyst Tab.</p>
                      </div>
                      ...
                      <div className="bg-[#0b1424] p-5 rounded-2xl border border-teal-500/20 space-y-3">
                        ...
                        <div className="bg-slate-950 p-3 rounded-lg border border-teal-500/10 text-[11px] text-slate-300 leading-relaxed">
                          💡 <strong>Analisis Terpadu:</strong> Dengan perpaduan 11 sensor lengkap di atas, tim analis pelabuhan dapat dengan mudah mengekspor laporan korelasi cuaca langsung ke format file <strong>Excel/CSV</strong> melalui
                          sub-menu yang terintegrasi di bagian bawah halaman.
                        </div>
                      </div>
                    </div>
                  </div>
                ...
                {activeManualChapter === "option" && (
                  <div className="space-y-6">
                    <div className="space-y-2">
                      ...
                      <p className="text-xs text-slate-300 leading-relaxed">
                        Halaman <strong>Option / Settings</strong> merupakan pusat konfigurasi utama untuk menghubungkan konsol visualisasi dengan perangkat keras fisik AWS, mengatur sinkronisasi data awan (cloud), mengkalibrasi posisi
                        fisik dermaga, serta mengatur ambang batas peringatan dini (alarm thresholds).
                      </p>
                    </div>

                    {/* Section 1: Data Input Methods */}
                    <div className="bg-[#0b1424] p-5 rounded-2xl border border-amber-500/20 space-y-3">
                      <h4 className="text-xs font-bold text-amber-400 uppercase tracking-wider flex items-center gap-1.5">🔌 1. PILIHAN KONEKSI DATA MASUK (SERIAL COM vs TCP/IP MOXA)</h4>
                      <p className="text-[11px] text-slate-300 leading-relaxed">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-[11px] text-slate-300">
                        <div className="bg-slate-950/60 p-3 rounded-lg border border-white/5 space-y-1.5">
                          ...
                          <ul className="list-disc pl-4 mt-1 space-y-1 text-slate-400">
                            <li>
                              <strong>Web Serial API Integration:</strong> Sistem memanfaatkan antarmuka native browser modern untuk membuka koneksi port tanpa software tambahan.
                            </li>
                            <li>
                              <strong>Baudrate & COM:</strong> Konfigurasikan nomor port komunikasi (misal: <code>COM3</code> atau <code>ttyUSB0</code>) dan Baudrate (paling umum <code>9600</code> bps atau <code>115200</code> bps).
                            </li>
                            <li>
                              <strong>Tombol Koneksi:</strong> Klik <span className="text-emerald-400">CONNECT COM PORT</span> untuk mengaktifkan pemindaian. Indikator port akan menyala hijau cerah saat aktif.
                            </li>
                          </ul>
                        </div>
                        ...
                        <div className="bg-slate-950/60 p-3 rounded-lg border border-white/5 space-y-1.5">
                          ...
                          <ul className="list-disc pl-4 mt-1 space-y-1 text-slate-400">
                            <li>
                              <strong>IP & Port Gateway:</strong> Tentukan IP Address Moxa (IP default: <code>172.16.4.48</code>) beserta Socket Port tujuan (umumnya <code>5001</code>).
                            </li>
                            <li>
                              <strong>Moxa Web Daemon:</strong> Komunikasi data dikawal oleh background daemon Websocket lokal (default pada port <code>8080</code>).
                            </li>
                            <li>
                              <strong>CORS & HTTPS Warn:</strong> Jika server dibuka melalui protokol HTTPS (Secure), browser akan memblokir koneksi HTTP tidak aman ke localhost. Pastikan Anda mengaktifkan izin{" "}
                              <em>Insecure Content / Allow</em> di setingan privasi browser Anda.
                            </li>
                          </ul>
                        </div>
                      </div>
                    ...
                    <div className="bg-[#0b1424] p-5 rounded-2xl border border-blue-500/20 space-y-3">
                      <h4 className="text-xs font-bold text-[#00f0ff] uppercase tracking-wider flex items-center gap-1.5">🧪 2. SETTING ALARM THRESHOLDS (BATAS AMBANG AMAN SENSOR)</h4>
                      <p className="text-[11px] text-slate-300 leading-relaxed">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-[11px] text-slate-300">
                        <div className="bg-slate-950/60 p-3 rounded-lg border border-white/5 space-y-1.5">
                          ...
                          <ul className="list-disc pl-4 mt-1 space-y-1 text-slate-400">
                            <li>
                              <strong>Min Safe pH (Acidic):</strong> Standar disetel pada angka <strong className="text-pink-400">6.5</strong>. Pembacaan di bawah ini memicu alarm asam.
                            </li>
                            <li>
                              <strong>Max Safe pH (Alkaline):</strong> Standar disetel pada angka <strong className="text-pink-400">8.5</strong>. Pembacaan di atas ini menandakan pencemaran basa.
                            </li>
                          </ul>
                        </div>
                        ...
                        <div className="bg-slate-950/60 p-3 rounded-lg border border-white/5 space-y-1.5">
                          ...
                          <ul className="list-disc pl-4 mt-1 space-y-1 text-slate-400">
                            <li>
                              <strong>Heavy Rain Threshold (mm):</strong> Nilai ambang batas standar disetel pada <strong className="text-sky-400">10.0 mm</strong>. Indikator curah hujan di dashboard utama akan otomatis berkedip oranye jika
                              curah hujan melampaui batas ini untuk memicu kesiapan operasional darurat.
                            </li>
                          </ul>
                        </div>
                      </div>
                    ...
                    <div className="bg-[#0b1424] p-5 rounded-2xl border border-emerald-500/20 space-y-3">
                      <h4 className="text-xs font-bold text-emerald-400 uppercase tracking-wider flex items-center gap-1.5">📡 3. PEMILIHAN MODE CLOUD (SINKRONISASI TRANSMISI INTERNET)</h4>
                      <p className="text-[11px] text-slate-300 leading-relaxed">Sistem AWS dilengkapi mesin transmisi multi-protokol untuk menyalurkan data telemetri pelabuhan secara simultan ke pusat kendali jarak jauh (Cloud Server):</p>
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-3 text-[10px] text-slate-300">
                        ...
                        <div className="bg-slate-950/60 p-2.5 rounded-lg border border-white/5 space-y-1">
                          ...
                          Mengirimkan data instan melalui protokol IoT berlatensi rendah ke broker MQTT. Membutuhkan konfigurasi Host Broker, Port (umumnya <code>1883</code>), Topic Publikasi (misal: <code>aws/ports/telemetry</code>),
                          beserta username/password otentikasi.
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="bg-[#0b1424] p-5 rounded-2xl border border-pink-500/20 space-y-3">
                      <h4 className="text-xs font-bold text-pink-400 uppercase tracking-wider flex items-center gap-1.5">🧭 4. KALIBRASI SUDUT DERMAGA (VISUAL PIER ANGLE CALIBRATION)</h4>
                      <p className="text-[11px] text-slate-300 leading-relaxed">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-[11px] text-slate-400">
                        <div className="space-y-1">
                          ...
                          Dermaga di setiap pelabuhan memiliki arah hadap pantai yang berbeda-beda secara geografis (azimuth). Dengan mengisi sudut kelurusan dermaga fisik (contoh: <code>45°</code> untuk dermaga berorientasi Timur Laut),
                          visual kompas di layar utama akan berputar secara akurat meluruskan posisi Kiri (Port) dan Kanan (Starboard) kapal terhadap garis pantai.
                        </div>
                        <div className="space-y-1">
                          <strong className="text-white block">B. Kalkulasi Vektor Angin Relatif (Relative Wind)</strong>
                          Setelah dikalibrasi, sistem secara otomatis menghitung komponen gaya angin: <strong>Crosswind</strong> (angin yang mendorong lambung kapal dari samping) dan <strong>Headwind/Tailwind</strong> (angin sejajar haluan
                          kapal) sehingga petugas pandu tahu persis tingkat kesulitan sandar kapal secara presisi.
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="bg-[#0b1424] p-5 rounded-2xl border border-teal-500/20 space-y-3">
                      <h4 className="text-xs font-bold text-teal-400 uppercase tracking-wider flex items-center gap-1.5">📟 5. RAW STREAM MONITOR & DYNAMIC CHANNEL MAPPING INDEXES</h4>
                      <p className="text-[11px] text-slate-300 leading-relaxed">
                        Data mentah yang keluar dari logger fisik biasanya dikirimkan berupa susunan string CSV satu baris yang dipisahkan karakter pembatas (Splitter Char). Menu Option memberikan alat penata (mapping tool) yang sangat
                        canggih:
                      </p>
                      <ul className="text-[11px] text-slate-300 space-y-2 list-disc pl-5">
                        <li>
                          <strong>Raw Stream Monitor Console:</strong> Terminal konsol hijau di kanan bawah menampilkan feed asupan biner asli yang tertangkap. Struktur paket data standar:{" "}
                          <code className="text-emerald-400 bg-slate-950 px-1 py-0.5 rounded">#AWS001;03-07-2026;08:43:55;30.2;78;220;5.4;12.4</code>
                        </li>
                        <li>
                          <strong>Dynamic Channel Mapping Indexes:</strong> Pengguna dapat mencocokkan nomor index (kolom pembacaan dimulai dari index 0) ke nama parameter sensor secara langsung. Misalnya, jika data kelembaban udara berada
                          di kolom ke-9, cukup pilih angka <code>9</code> pada kolom <strong>Humidity (ch_8)</strong>. Angka real-time sensor akan langsung diperbarui saat itu juga tanpa menghentikan sistem monitoring!
                        </li>
                        <li>
                          <strong>Moxa Telemetry Presets:</strong> Tombol pintas sekali klik <strong className="text-cyan-400">LOAD MOXA TELEMETRY PRESETS</strong> akan otomatis memuat setingan urutan index bawaan stasiun Moxa AWS lapangan
                          standar untuk kemudahan instalasi kilat.
                        </li>
                      </ul>
                    </div>
                      ...
                      <div className="border border-white/10 rounded-2xl bg-slate-950/60 p-4 space-y-3 shadow-inner">
                        ...
                        <div className="grid grid-cols-12 gap-3 aspect-[1.8/1] text-[8px] font-mono text-slate-400">
                          ...
                          <div className="col-span-6 bg-[#0b1424] border border-white/5 rounded-lg p-2 space-y-1.5">
                            ...
                            <div className="bg-emerald-500/10 p-1 rounded text-emerald-400 text-[7px]">Status: Connected (IP 172.16.4.48:5001)</div>
                          ...
                          <div className="col-span-6 bg-[#0b1424] border border-white/5 rounded-lg p-2 flex flex-col justify-between">
                            ...
                            <div className="border border-white/5 rounded bg-black p-1 font-mono text-[5px] text-emerald-400 leading-none">[14:47:53 RAW] "#AWS001;28-06-2026;14:47:52;30.2;29.2;..."</div>
                          </div>
                        </div>
                        <p className="text-[10px] text-slate-400 italic">* Tampilan skema menu Option di atas mencakup seluruh widget pengaturan sesuai screenshot Option Tab.</p>
                      </div>
                    </div>
                  </div>
                )}
                ...
                {activeManualChapter === "bmkg" && (
                  <div className="space-y-6">
                    ...
                    <div className="bg-[#0b1424] p-5 rounded-2xl border border-emerald-500/20 space-y-3">
                      <h4 className="text-xs font-bold text-emerald-400 uppercase tracking-wider flex items-center gap-1.5">📡 1. KONTROL SYNC & SUMBER DATA (BMKG LIVE SYNC)</h4>
                      <p className="text-[11px] text-slate-300 leading-relaxed">Di bagian kanan atas header, terdapat serangkaian kontrol interaktif untuk mengelola asupan data prakiraan maritim yang diterima sistem:</p>
                      <ul className="text-[11px] text-slate-300 space-y-2 list-disc pl-5">
                        <li>
                          <strong>Sync Live Button:</strong> Klik tombol hijau <strong className="text-emerald-400">SYNC LIVE</strong> untuk memaksa sistem melakukan panggilan API (fetch) langsung ke server BMKG Maritim untuk memperbarui
                          seluruh data prakiraan jam terbaru secara online.
                        </li>
                        <li>
                          ...
                          <ul className="list-disc pl-5 mt-1 space-y-1 text-slate-400">
                            <li>
                              <span className="text-indigo-400 font-bold">live:</span> Data berhasil ditarik secara langsung dan instan dari server BMKG pusat.
                            </li>
                            <li>
                              <span className="text-blue-400 font-bold">cache:</span> Data dimuat dari memori lokal berkecepatan tinggi demi efisiensi bandwidth transmisi.
                            </li>
                            <li>
                              <span className="text-amber-400 font-bold">stale-cache / static:</span> Terjadi kendala internet, sistem menyajikan data statis cadangan terenkripsi agar konsol monitor tidak kosong.
                            </li>
                          </ul>
                        </li>
                        <li>
                          <strong>Pencarian Data (Cari Jam / Cuaca):</strong> Kotak pencarian interaktif yang menyaring isi tabel prakiraan secara instan berdasarkan kata kunci tertentu (misalnya Anda mencari <code>"14:00"</code>,{" "}
                          <code>"Hujan"</code>, atau arah angin <code>"Utara"</code>).
                        </li>
                        <li>
                          <strong>Layout Selector (Table vs Grid):</strong> Tombol toggle untuk beralih mode visualisasi data antara format <strong>Table</strong> (tabel spreadsheet presisi tinggi) atau format <strong>Grid</strong>{" "}
                          (kartu-kartu ringkas modern yang responsif).
                        </li>
                      </ul>
                    </div>
                    ...
                    <div className="bg-[#0b1424] p-5 rounded-2xl border border-blue-500/20 space-y-3">
                      <h4 className="text-xs font-bold text-sky-400 uppercase tracking-wider flex items-center gap-1.5">⚓ 2. PORT PROFILE SELECTOR (PEMILIHAN LOKASI PELABUHAN)</h4>
                      <p className="text-[11px] text-slate-300 leading-relaxed">Sistem AWS mengawal wilayah perairan Selat Sunda dan Teluk Jakarta dengan menyediakan preset profil pelabuhan BMKG yang dapat dialihkan secara instan:</p>
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-[11px] text-slate-300">
                        <div className="bg-slate-950/60 p-3 rounded-lg border border-white/5 space-y-1">
                      ...
                      <p className="text-[11px] text-slate-400 leading-relaxed">
                        💡 <strong>Mode Profil Kustom (Custom Port):</strong> Jika Anda memilih opsi <em>── LAINNYA / CUSTOM ──</em> pada dropdown, sistem membuka kolom input untuk mendefinisikan <strong>Custom Slug</strong> dan{" "}
                        <strong>Custom Name</strong> secara mandiri guna menyambungkan stasiun cuaca pelabuhan khusus Anda sendiri ke dalam konsol.
                      </p>
                    </div>

                    {/* Section 3: Sensor Information Overview */}
                    <div className="bg-[#0b1424] p-5 rounded-2xl border border-pink-500/20 space-y-3">
                      <h4 className="text-xs font-bold text-pink-400 uppercase tracking-wider flex items-center gap-1.5">📈 3. INFORMASI SENSOR SINKRON & INDIKATOR UTAMA</h4>
                      <p className="text-[11px] text-slate-300 leading-relaxed">Bagian tengah menampilkan 4 panel ringkasan parameter laut utama hasil komputasi data BMKG terbaru:</p>
                      <div className="grid grid-cols-1 md:grid-cols-4 gap-3 text-[10px] text-slate-300">
                        ...
                        <div className="bg-slate-950/60 p-2.5 rounded-lg border border-white/5 space-y-1">
                          ...
                          Menampilkan nilai elevasi pasang air laut dermaga (m). Indikator status akan otomatis mendeteksi kondisi <strong className="text-pink-400">"PASANG TINGGI"</strong> jika elevasi melampaui batas aman draf kapal
                          sandar.
                        </div>
                        <div className="bg-slate-950/60 p-2.5 rounded-lg border border-white/5 space-y-1">
                    ...
                    <div className="bg-[#0b1424] p-5 rounded-2xl border border-yellow-500/20 space-y-3">
                      <h4 className="text-xs font-bold text-yellow-500 uppercase tracking-wider flex items-center gap-1.5">📊 4. INTERACTIVE TREND CHARTS (VISUALISASI SIKLUS 12 JAM)</h4>
                      <p className="text-[11px] text-slate-300 leading-relaxed">Sistem mengolah seluruh deretan data prakiraan BMKG ke dalam dua grafik interaktif cerdas beresolusi tinggi berbasis pustaka Recharts:</p>
                      <ul className="text-[11px] text-slate-300 space-y-1.5 list-disc pl-5">
                        <li>
                          <strong>Grafik Elevasi Air & Gelombang (Area Chart):</strong> Menggabungkan grafik pasang air laut berwarna pink gradien dengan grafik tinggi gelombang berwarna hijau emerald guna membantu analis melihat korelasi
                          antara pasang surut dengan kenaikan gelombang dermaga.
                        </li>
                        <li>
                    ...
                    <div className="bg-[#0b1424] p-5 rounded-2xl border border-teal-500/20 space-y-3">
                      <h4 className="text-xs font-bold text-teal-400 uppercase tracking-wider flex items-center gap-1.5">🧭 5. DETAIL HOUR INSPECTOR & SAFETY ASSESSMENT (ASESMEN KESELAMATAN)</h4>
                      <p className="text-[11px] text-slate-300 leading-relaxed">
                        Saat operator mengeklik salah satu baris jam prakiraan pada tabel/grid di sebelah kiri, panel <strong>Selected Hour Inspector</strong> di bagian kanan akan melakukan pembongkaran instrumen visual parameter laut
                        secara mendalam:
                      </p>
                      <ul className="text-[11px] text-slate-300 space-y-2 list-disc pl-5">
                        <li>
                        ...
                        <li>
                          <strong>Port Commander Safety Assessment:</strong> Fitur unggulan berupa kalkulasi matematis otomatis untuk menghasilkan laporan kesimpulan keselamatan operasional bersandarnya kapal kargo di pelabuhan aktif secara
                          dinamis.
                        </li>
                      </ul>
                    </div>
                      ...
                      <div className="border border-white/10 rounded-2xl bg-slate-950/60 p-4 space-y-3 shadow-inner">
                        ...
                        <div className="grid grid-cols-12 gap-3 aspect-[1.8/1] text-[8px] font-mono text-slate-400">
                          ...
                          <div className="col-span-4 bg-[#050b14] border border-emerald-500/20 rounded p-1.5 space-y-1 text-[5px]">
                            ...
                            <div className="bg-emerald-950/40 border border-emerald-500/10 p-1 rounded text-emerald-200">Asesmen: Aman untuk penyandaran kapal laut.</div>
                          </div>
                        </div>
                        <p className="text-[10px] text-slate-400 italic">* Tampilan skema halaman BMKG di atas mencakup seluruh widget parameter cuaca laut sesuai screenshot BMKG Tab.</p>
                      </div>
                    </div>
                  </div>
                )}
                ...
                {activeManualChapter === "database" && (
                  <div className="space-y-6">
                    <div className="space-y-2">
                      ...
                      <p className="text-xs text-slate-300 leading-relaxed">Bagian ini menjelaskan secara rinci tentang pengolahan logs dan cara mengekspor data logger AWS ke dalam format tabel excel (CSV).</p>
                    </div>
                    ...
                    <div className="bg-gradient-to-r from-emerald-950/40 to-transparent p-5 rounded-2xl border border-emerald-500/20 space-y-3">
                      ...
                      <ol className="text-[11px] text-slate-200 space-y-2.5 list-decimal pl-5">
                        ...
                        <li>
                          Pada panel <strong>Mode Penampilan Data Telemetri</strong>, pilih sumber data log yang diinginkan. Anda dapat men-toggle mode ke <strong>ONLINE TABLE</strong> untuk melihat logs riil dari database utama, atau{" "}
                          <strong>OFFLINE LOGS</strong> untuk melihat database simulasi offline.
                        </li>
                        <li>
                          ...
                          <ul className="list-disc pl-5 mt-1 space-y-1 text-slate-300">
                            <li>
                              <strong>START LOG PERIOD:</strong> Tanggal mulai penarikan data log.
                            </li>
                            <li>
                              <strong>END LOG PERIOD:</strong> Tanggal akhir penarikan data log.
                            </li>
                          </ul>
                        </li>
                        <li>
                        ...
                        <li>
                          Sistem akan memformat data logs ke dalam struktur spreadsheet dan otomatis mengunduh file berformat <code>.csv</code> ke komputer Anda. File ini siap dibuka langsung menggunakan program spreadsheet seperti{" "}
                          <strong>Microsoft Excel</strong> atau <strong>Google Sheets</strong> untuk analisis teknis lanjutan.
                        </li>
                      </ol>
                    </div>
                    ...
                    <div className="bg-white/5 p-4 rounded-xl border border-white/5 space-y-2 text-[11px]">
                      ...
                      <p className="text-slate-300 leading-relaxed">
                        Sistem AWS memiliki fitur <strong>Integrated Database Engine</strong> yang berjalan secara efisien. Mengacu pada standar WMO (World Meteorological Organization), sistem ini melakukan kompresi{" "}
                        <em>10-Minute Average</em> (Rata-rata 10 menit) dari 5 sampel sensor instan yang ditangkap buffer akumulator. Tombol <strong>SYNC & AMBIL DATA ASLI</strong> digunakan untuk melakukan sinkronisasi database server
                        secara real-time.
                      </p>
                    </div>
                    ...
                    <div className="border-t border-white/5 pt-4">
                      ...
                      <div className="border border-white/10 rounded-2xl bg-slate-950/60 p-4 space-y-3 shadow-inner">
                        ...
                        <p className="text-[10px] text-slate-400 italic">* Tampilan skema menu Database di atas mencakup seluruh widget pengolahan logs sesuai screenshot Database Tab.</p>
                      </div>
                    </div>
                  </div>
                )}
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="bg-[#0b1424] p-5 rounded-2xl border border-red-500/20 space-y-4">
                      ...
                      <div className="space-y-3.5 text-[11px] text-slate-300">
                        <div className="border-l-2 border-red-500 pl-3 space-y-1">
                          ...
                          <p className="leading-relaxed">
                            <span className="text-red-400 font-semibold">Fungsi Warning:</span> Jika pH terdeteksi di luar batas aman (misal kurang dari 6.5 karena polusi asam, atau lebih dari 8.5 karena limpahan limbah kimia), teks status
                            kualitas air pada dashboard akan berubah dari hijau <strong>"IDEAL"</strong> menjadi merah berkedip bertuliskan <strong className="text-red-400">"WARNING / DANGER"</strong>.
                          </p>
                        </div>
                        ...
                        <div className="border-l-2 border-amber-500 pl-3 space-y-1">
                          ...
                          <p className="leading-relaxed">
                            <span className="text-amber-400 font-semibold">Fungsi Warning:</span> Hujan yang terlalu lebat (&gt; 10 mm) dapat menghalangi pandangan nakhoda kapal dan memicu genangan air di dermaga. Indikator curah hujan akan
                            berkedip oranye jika curah hujan melampaui batas aman ini.
                          </p>
                        </div>
                        ...
                        <div className="border-l-2 border-cyan-500 pl-3 space-y-1">
                          ...
                          <p className="leading-relaxed">
                            <span className="text-amber-400 font-semibold">Parameter / Batas Aman:</span> Mengukur kecepatan angin sesaat maksimum (<strong className="text-cyan-400">WIND MAX</strong>) dan melacak kejadian hembusan angin
                            kencang mendadak (<strong className="text-cyan-400">GUST EVENT</strong>).
                          </p>
                          <p className="leading-relaxed">
                            <span className="text-red-400 font-semibold">Fungsi Warning:</span> Kecepatan angin di atas <strong className="text-red-400">15 m/s</strong> digolongkan sebagai bahaya badai laut. Jika sensor mencatat angka ini,
                            status Gust akan berkedip merah untuk memperingatkan operator bahwa kondisi luar ruangan sangat berbahaya untuk bongkar muat gantry crane dan proses penyandaran kapal.
                          </p>
                        </div>
                        ...
                        <div className="border-l-2 border-purple-500 pl-3 space-y-1">
                          ...
                          <p className="leading-relaxed">
                            <span className="text-purple-400 font-semibold">Fungsi Warning:</span> Jika tingkat risiko melampaui 50%, indikator status operasi di dashboard analis akan bergeser dari hijau <strong>"SAFE OPERATION"</strong> ke
                            merah bertuliskan <strong>"HIGH MARINE RISK"</strong> untuk mengisyaratkan kesiagaan kru darurat pelabuhan.
                          </p>
                        </div>
                        ...
                        <div className="border-l-2 border-orange-500 pl-3 space-y-1">
                          ...
                          <p className="leading-relaxed">
                            <span className="text-amber-400 font-semibold">Parameter / Batas Aman:</span> Menguji jeda asupan (timeout) penerimaan string data telemetri. Batas kritis ditoleransi selama maksimal{" "}
                            <strong className="text-orange-400">10-15 detik</strong> tanpa pembaruan data masuk.
                          </p>
                          <p className="leading-relaxed">
                            <span className="text-red-400 font-semibold">Fungsi Warning:</span> Jika transmisi terputus akibat kabel kendur atau daya Moxa mati, widget indikator koneksi di panel atas akan berubah menjadi merah menyala
                            bertuliskan <strong className="text-red-400">"DATA FEED DISCONNECTED / OFFLINE"</strong> dan mengunci visualisasi data pada status terakhir. Ini merupakan protokol keamanan kritis untuk mencegah kesalahan
                            pembacaan data basi/stale oleh petugas pelabuhan.
                          </p>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div className="bg-white/5 p-4 rounded-xl border border-white/5 space-y-2 text-[11px]">
                        ...
                        <ul className="space-y-2 text-slate-300 list-disc pl-4">
                          <li>
                            ...
                            <br />
                            <em>Langkah Penanganan:</em> Periksa panel <strong>Moxa Live Status</strong> di menu Option. Pastikan daemon berstatus <strong>CONNECTED</strong>. Jika terputus, pastikan kabel LAN Moxa terhubung dan alamat IP
                            Moxa <code>172.16.4.48</code> dalam kondisi menyala (aktif).
                          </li>
                          <li>
                            ...
                            <ul className="list-decimal pl-4 mt-1 space-y-1 text-slate-400">
                              <li>
                                Buka tab <strong>OPTION</strong> dan periksa feed data mentah di console bawah. Jika kosong, asupan fisik terhenti.
                              </li>
                              ...
                              <li>
                                Jika memakai mode Moxa TCP/IP: lakukan ping ke IP <code>192.168.1.254</code> (atau IP yang diset) lewat Command Prompt PC Anda untuk memastikan link jaringan hidup.
                              </li>
                          ...
                          <li>
                            ...
                            <br />
                            <em>Langkah Penanganan:</em> Terjadi kesalahan urutan parsing data. Masuk ke menu Option dan atur ulang nomor index pada panel <strong>Sensor Channel Mapping Indexes</strong> untuk menyelaraskan urutan kolom
                            output dari logger AWS Anda.
                          </li>
                        </ul>
                      </div>
                      ...
                      <div className="bg-amber-500/10 border border-amber-500/20 p-4 rounded-xl text-[11px] space-y-1.5 text-slate-300">
                        ...
                        <p className="leading-relaxed">
                          Pastikan Moxa NPort Gateway dipasang dalam kotak panel IP66 yang kedap air dari paparan air laut asin (salt mist/korosi). Lakukan restart berkala pada router Moxa melalui web interface admin atau mematikan steker
                          listrik selama 10 detik apabila koneksi daemon mengalami penurunan kecepatan atau kehilangan transmisi paket data (packet loss).
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="bg-gradient-to-r from-[#0d1f38] via-[#091629] to-[#0d1f38] p-5.5 rounded-2xl border border-[#00f0ff]/30 space-y-4">
                      ...
                      <p className="text-[11px] text-slate-300 leading-relaxed">
                        Sistem kini dilengkapi dengan modul <strong>EWS (Early Warning System) Audio Synthesizer</strong> berbasis Web Audio API dan Speech Synthesis bawaan berstandar industri. Modul ini membunyikan alarm otomatis secara
                        real-time apabila parameter penting melampaui ambang batas bahaya, memberikan panduan audio yang cepat untuk operator lapangan.
                      </p>
                          ...
                          <ul className="space-y-2 list-none">
                            ...
                            <li className="flex items-start gap-1.5">
                              ...
                              <div>
                                ...
                                <p className="text-[10px] text-slate-400 mt-0.5">Suara frekuensi menyapu naik-turun dari 600 Hz ke 1000 Hz, dirancang khusus untuk mensimulasikan sirene peringatan badai ekstrim (tornado/siaga tinggi).</p>
                              </div>
                            </li>
                            <li className="flex items-start gap-1.5">
                              ...
                              <div>
                                ...
                                <p className="text-[10px] text-slate-400 mt-0.5">Suara bip tunggal terus menerus (1.8 detik) pada gelombang kotak (square wave) berkepadatan tinggi untuk memberikan peringatan keras.</p>
                              </div>
                            </li>
                            <li className="flex items-start gap-1.5">
                              ...
                              <div>
                                ...
                                <p className="text-[10px] text-slate-400 mt-0.5">Seri 3 bip pendek berkepala tajam, ideal sebagai penanda awal bahaya ringan atau peringatan periodik.</p>
                              </div>
                            </li>
                            <li className="flex items-start gap-1.5">
                        ...
                        <div className="bg-slate-950/60 p-4 rounded-xl border border-white/5 space-y-2.5">
                          ...
                          <div className="space-y-2">
                            <div className="border-l border-[#00f0ff]/30 pl-2">
                              ...
                              <p className="text-[10px] text-slate-400 mt-0.5">
                                Muncul di halaman utama jika batas aman dilampaui. Menyediakan opsi penonaktifan instan (Master Speaker Toggle) dan tombol senyap sementara (Silence Alarm) untuk mematikan suara secara cepat tanpa mereset
                                sistem deteksi.
                              </p>
                            </div>
                            ...
                            <div className="border-l border-[#00f0ff]/30 pl-2">
                              ...
                              <p className="text-[10px] text-slate-400 mt-0.5">
                                Operator dapat menguji langsung setiap jenis suara di panel <strong>Speaker Diagnostic</strong> (Uji TTS, Uji Bip Panjang, Uji Ambulans, dsb), serta mengatur penggeser tingkat volume suara (0% s/d 100%)
                                sesuai kebutuhan bising lapangan pelabuhan.
                              </p>
                            </div>
                            ...
                            <div className="border-l border-[#00f0ff]/30 pl-2">
                              ...
                              <p className="text-[10px] text-slate-400 mt-0.5">
                                Untuk mencegah penumpukan audio yang mengganggu, laporan verbal diatur dengan sistem jeda minimal 12 detik.{" "}
                                <strong>Sistem alarm otomatis berhenti berbunyi dan berkedip seketika ketika hujan berhenti atau parameter kembali normal.</strong>
                              </p>
                            </div>
                            ...
                            <div className="border-l border-emerald-500/30 pl-2">
                              ...
                              <p className="text-[10px] text-slate-400 mt-0.5">
                                Pemetaan sensor (Sensor Channel Mapping) kini disimpan otomatis secara langsung dan tidak ter-reset saat refresh. Format data pengiriman HTTP API dikirim menggunakan format string terstandardisasi:{" "}
                                <code className="text-emerald-400 font-mono text-[9px] bg-slate-950 px-1">ID;timestamp(utc);temp avg;temp min;temp max;rh;kec angin;arah angin;radiasi;tekanan;water level;temperatur air;phair;hujan;</code>{" "}
                                demi performa integrasi tinggi.
                              </p>
                            </div>
                          </div>
                        </div>
            ...
            <div className="bg-slate-950 px-6 py-4 border-t border-white/10 flex flex-col sm:flex-row justify-between items-center gap-3 shrink-0 text-slate-400 text-xs font-mono">
              ...
              <button onClick={() => setIsManualModalOpen(false)} className="bg-[#00f0ff] hover:bg-[#00d0e0] text-black font-bold px-6 py-2 rounded-xl transition-all cursor-pointer shadow-[0_0_15px_rgba(0,240,255,0.3)] text-xs font-sans">
                TUTUP BUKU MANUAL
              </button>
            </div>
          </div>
          ...
          <div className="bg-[#07111e] border-2 border-[#00f0ff]/30 w-full max-w-md rounded-3xl shadow-[0_0_60px_rgba(0,240,255,0.4)] flex flex-col overflow-hidden relative text-slate-100">
            ...
            <div className="bg-gradient-to-r from-[#11243b] to-[#081220] px-6 py-5 border-b border-[#00f0ff]/20 flex justify-between items-center shrink-0">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-md font-black tracking-wider uppercase text-white">{adminModalMode === "login" ? "LOGIN OTORISASI ADMIN" : "REGISTRASI ADMIN BARU"}</h2>
              ...
              {adminModalMode === "login" ? (
                <form onSubmit={handlePasscodeSubmit} className="space-y-4">
                  ...
                  <div className="space-y-3">
                    ...
                    {passcodeError && <p className="text-[10px] font-mono text-rose-400 bg-rose-500/10 border border-rose-500/20 p-2 rounded-lg text-center font-bold">⚠️ {passcodeError}</p>}
                  </div>
                  ...
                  <div className="text-center bg-black/40 p-2.5 rounded-xl border border-white/5">
                    ...
                    <span className="text-[10px] font-mono text-slate-400 mt-1 block">
                      Default: <strong className="text-teal-400 font-bold">admin</strong> / passcode: <strong className="text-teal-400 font-bold">admin123</strong>
                    </span>
                  </div>
                ...
                <form onSubmit={handleRegisterAdmin} className="space-y-4">
                  <div className="space-y-1 text-center py-1">
                    <p className="text-xs text-slate-300 leading-relaxed">Buat kredensial administrator baru secara instan di bawah ini untuk mengelola parameter sistem stasiun AWS KBS.</p>
                  </div>
                  ...
                  <div className="space-y-3">
                    ...
                    {registerError && <p className="text-[10px] font-mono text-rose-400 bg-rose-500/10 border border-rose-500/20 p-2 rounded-lg text-center font-bold">⚠️ {registerError}</p>}
                  </div>
